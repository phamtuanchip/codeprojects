...
<title>Nhập hệ số Alpha T</title>
<!-- InstanceEndEditable -->
                ...
                <tr>
				  ...
				  <script language="javascript">
					    ...
					    function kiemtra(diem)
						{
							...
							for (i=1; i<= diem; i ++)
							{
								tong= parseFloat(tong)+parseFloat(document.getElementById("alphat"+i).value)
								//alert(i);
							}
							return tong
						}
  ...
  for (i=0; i<(args.length-2); i+=3) { test=args[i+2]; val=MM_findObj(args[i]);
    if (val) { nm=val.name; if ((val=val.value)!="") {
      if (test.indexOf('isEmail')!=-1) { p=val.indexOf('@');
        ...
        if (isNaN(val)) errors+='- '+nm+' phải nhập số.\n';
        if (test.indexOf('inRange') != -1) { p=test.indexOf(':');
          min=test.substring(8,p); max=test.substring(p+1);
          if (num<min || max<num) errors+='- '+nm+' phải bằng  '+min+'\n';
    } } } else if (test.charAt(0) == 'R') errors += '- '+nm+' is required.\n'; }
  } if (errors) alert('Bạn phải nhập lại vì :\n'+errors);
...
//-->
				 ...
				 <%
					...
					for i =1 to tonum
					namealpha="alphat"&i
					namebuocgc="mabuocgc"&i
					sqlupdate= "update buocgiacong set alphat="& Request.Form(namealpha)&" where  mabuocgiacong="&Request.Form(namebuocgc)&" and status=1 "
				 ...
				 %>
			      ...
			      <p>&nbsp;</p>
			      <p>Nhập hệ số alpha T  của sản phẩm:</p>
				  <% rs.open "select * from sanpham where masanpham ="&Session("ma_sp")& " and status=1 ",conn%>
				  <p><%=rs("tensanpham")%>&nbsp; </p>
				  <%rs.close()%>
				  <form action="Nhap_alphat.asp" method="post" name="danhsachbgc" id="danhsachbgc" onsubmit="MM_validateForm('Tổng alpha T','','RinRange1:1');return document.MM_returnValue" >
				    <table width="100%" border="1">
                      <tr>
                        <td>&nbsp;</td>
                        <td>Hệ số anpha T </td>
                      </tr>
                     <% rs.open "select * from buocgiacong where masanpham ="& Session("ma_sp"),conn%>
					 <% dem=1
					  	while not rs.eof 
					  %>
					  <tr>
                        <td><%=rs("tenbuocgiacong")%>&nbsp;
                          <input name="mabuocgc<%=dem%>" type="hidden" id="mabuocgc<%=dem%>"  value="<%=rs("mabuocgiacong")%>"/></td>
                        <td><input name="alphat<%=dem%>" type="text" id="alphat<%=dem%>" value="<%=rs("alphat")%>" size="3" /></td>
                      </tr>
					  <%dem = dem +1%>
					  <%rs.movenext()%>
					  <%wend %>
                      ...
                      <tr>
                        <td>
                          <div align="right">
                            <input type="submit" name="Submit" value="  Nhập  "  onmouseover="document.getElementById('Tổng alpha T').value=kiemtra(<%=dem-1%>)"/>
                            </div></td>
                        <td><input name="Tổng alpha T" type="text" id="Tổng alpha T"  size="3" readonly="true" />
