...
<title>Sửa các chỉ tiêu của doanh nghiệp</title>
<!-- InstanceEndEditable -->
...
<script language="javascript">
	function check()
	{
		if(form1.TenChiTieu.value=="")
		{
			alert("Bạn phải nhập tên chi tiết thông tin");
			form1.TenChiTieu.focus();
			return false;
		}
...
<%
	...
	if(loai="sua") then
		TenChiTieu = Request.QueryString("TenChiTieu")
		GhiChu = Request.QueryString("GhiChu")
		
		'Response.Write("sua de:"&TenChiTieu)
		sql = "UPDATE BangChiTieu SET TenChiTieu=N'"&TenChiTieu&"' , GhiChu=N'"&GhiChu&"' WHERE MaChiTieu="&MaChiTieu
		'Response.Write(sql)
		RS.Open sql,conn
		Response.Redirect("ThemChiTieu.asp?MaDoanhNghiep="&MaDoanhNghiep&"&TenDoanhNghiep="&TenDoanhNghiep)
	else
		'Response.Write("Hien form de sua")
		sql = "SELECT * FROM BangChiTieu WHERE MaChiTieu="&MaChiTieu
		RS.Open sql,conn
%>
<form id="form1" name="form1" method="get" action="SuaChiTieu.asp" onsubmit="return check();" >
  <table width="100%" border="1">
	<tr>
      <td width="120">T&ecirc;n ch&#7881; ti&ecirc;u </td>
      <td width="418"><input name="TenChiTieu" type="text" id="TenChiTieu" size="55"  value="<%=RS(1)%>" /></td>
    </tr>
    <tr>
      <td>Ghi ch&uacute; </td>
      <td><textarea name="GhiChu" cols="20" rows="5" id="GhiChu"><%=RS(3)%></textarea></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
	...
	<input type="hidden" name="MaChiTieu" value="<%=MaChiTieu%>" />
