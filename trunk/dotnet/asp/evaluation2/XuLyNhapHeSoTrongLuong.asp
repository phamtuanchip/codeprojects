          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  <%
					...
					Diem= Request.Form("txtDiem")
					GhiChu = Request.Form("GhiChu")
					MaHoiDong=Request.Form("MaHoiDong")
					
					Rs.open "Select sum(Diem) from HeSoTrongLuong where MaHoiDong="& MaHoiDong,conn
					'Response.Write(Rs(0))
					if (Diem+Rs(0))>1 then
						Response.Write("Điểm hệ số trọng lượng đã vượt quá 1. Bạn hãy kiểm tra lại điểm vừa nhập")
					
					else
					strSQL = "insert into HeSoTrongLuong (TenHeSoTrongLuong,Diem,GhiChu,MaHoiDong) values (N'"+TenHeSoTrongLuong + "'," + Diem + ",N'" + GhiChu + "',"+ MaHoiDong +")"
					conn.Execute strSQL
