...
<title>Bachkhoa mechatronics - Sửa điểm bước gia công</title>
<!-- InstanceEndEditable -->
...
<script language="javascript">
	function check(){
	document.all.formnhantomoitruong.mantmt.focus();
}
function validateForm(f){

	if(!istxtTenNhanToMoiTruongEmpty(f))
		return false
}
	
function istxtTenNhanToMoiTruongEmpty(formnhantomoitruong) {
      if ( formnhantomoitruong.tenntmt.value == "" ||
			formnhantomoitruong.tenntmt.value == null){
          alert("Bạn phải nhập tên nhân tố môi trường");
          return false;
       }
	   else {
          return true;
...
Dim RsSua
Dim str
str = "select BuocGiaCong.MaBuocGiaCong, BuocGiaCong.TenBuocGiaCong, SanPham.TenSanPham,HoiDong.TenHoiDong,AlPhaT,BuocGiaCong.GhiChu from BuocGiaCong inner join SanPham on BuocGiaCong.MaSanPham = SanPham.MaSanPham inner join HoiDong on BuocGiaCong.MaHoiDong = HoiDong.MaHoiDong where BuocGiaCong.MaBuocGiaCong = "& Request.QueryString("MaBuocGiaCong")
set RsSua = Server.CreateObject("ADODB.Recordset")
RsSua.Open str,conn
%>
<center><font face="Arial, Helvetica, sans-serif" color="#006633" size="4">Sửa điểm bước gia công</font>
<p>
Chú ý : Bạn không thể sửa những trường khóa có dấu (*)
</center>
<form name="formsuadiembuocgiacong" id="formsuadiembuocgiacong" method="post" action="CapNhatSuaBuocGC.asp" onSubmit="return validateForm(this)">
  <table width="418" border="0" align="center">
    <tr>
      <td width="172"></td>
      <td width="230"><input name="txtMaBuocGiaCong" type="hidden" id="mantmt" value="<%=RsSua("MaBuocGiaCong")%>" readonly>&nbsp;&nbsp;</td>
    </tr>
    <tr>
      <td>Tên bước gia công</td>
      <td><input name="txtTenBuocGiaCong" type="text" id="tenntmt" value="<%=RsSua("TenBuocGiaCong")%>" ></td>
    </tr>
   <tr>
      <td>Tên sản phẩm </td>
      <td><input name="txtTenSanPham" type="text" value="<%=RsSua("TenSanPham")%>" readonly="1">&nbsp;&nbsp;*</td>
    </tr>
	<tr>
	  <td>Tên hội đồng </td>
	  <td><input type="text" name="txtTenHoiDong" value="<%=RsSua("TenHoiDong")%>" readonly="1" />
	    *</td>
	  </tr>
	<tr>
	  <td>AlPha T</td>
	  <td><input type="text" name="txtAlPhaT" value="<%=RsSua("AlPhaT")%>" /></td>
	  </tr>
	
    <tr>
      <td align="right"><div align="left">Ghi ch&uacute;</div></td>
      <td><textarea name="txtghichu"  cols="30" rows="5"><%=RsSua("GhiChu")%></textarea></td>
    </tr>
    <tr>
