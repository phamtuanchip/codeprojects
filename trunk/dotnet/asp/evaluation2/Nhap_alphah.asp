...
//-->
				 ...
				 %>
                  ...
                  <p>&nbsp;</p>
                  <p>Nhập hệ số alpha H  của sản phẩm:</p>
                  <%
				   if len(Request.Form()) >0 then
					...
					sqlupdate= "update nhantoconnguoi set alphah="& Request.Form(namealpha)&" where  manhantoconnguoi="&Request.Form(namebuocgc)&""
                  ...
                  <p><%=rs("tensanpham")%>&nbsp; </p>
                  <%rs.close()%>
                  <form action="Nhap_alphah.asp" method="post" name="chitietthongtin" id="chitietthongtin" onsubmit="MM_validateForm('Tổng alpha H','','RinRange1:1');return document.MM_returnValue" >
                    <table width="100%" border="1">
                      <tr>
                        <td>&nbsp;</td>
                        <td>Hệ số anpha H </td>
                      </tr>
                      <% rs.open "select * from nhantoconnguoi where masanpham = "& Session("ma_sp"),conn%>
                      <% dem=1
					  	while not rs.eof 
					  %>
                      <tr>
                        <td height="0"><%=rs("tennhantoconnguoi")%>&nbsp;
                            <input name="machitiettt<%=dem%>" type="hidden" id="machitiettt<%=dem%>"  value="<%=rs("manhantoconnguoi")%>"/></td>
                        <td><input name="alphai<%=dem%>" type="text" id="alphai<%=dem%>" value="<%=rs("alphah")%>" size="3"/></td>
                      </tr>
                      <%dem = dem +1%>
                      <%rs.movenext()%>
                      <%wend %>
                      ...
                      <tr>
                        <td><div align="right">
                            <input name="Submit" type="submit"  onmouseover="document.getElementById('Tổng alpha H').value=kiemtra(<%=dem-1%>)" value="  Nhập  "/>
                        </div></td>
                        <td><input name="Tổng alpha H" type="text" id="Tổng alpha H"  size="3" readonly="true" />
