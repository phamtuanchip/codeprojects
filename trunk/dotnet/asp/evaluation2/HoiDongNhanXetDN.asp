...
{
		url="InPhieuHoiDongNhanxetDN.asp?MaDoanhNghiep="+form1.MaDoanhNghiep.value +"&MaSanPham="+form1.MaSanPham.value ;
                ...
                <tr>
				  ...
				  <!-- InstanceBeginEditable name="noidungcuanguoidung " -->
					...
					<% 
						...
						'Response.Write(SqlHoiDongNhanXetDoanhnghiep)
						
						'Response.Write(rsNhanXet("NhanXet"))
					%>
					<% 
						'if len(Request.QueryString("ThongBao"))>0 then
						'	Response.Write(Request.QueryString("ThongBao"))
						'end if
					 %>
                    <br />
				    ...
				    <table width="100%" border="0">
                      <tr>
                        <td colspan="2"><input name="bntInNhanXet" type="button" id="bntInNhanXet" value="In nhận xét" onclick='messageWindow("Nhận xét của hội đồng");'/></td>
                      </tr>
                      <tr>
                        <td colspan="2"><div align="center"><strong>Nhận xét của hội đồng chấm điểm </strong><br />
                          <br />
                        </div></td>
                      </tr>
                      <tr>
                        <td width="25%">Doanh nghiệp: </td>
                        <td width="75%">&nbsp;<%=rsDN(0) %>
                          <input type="hidden" name="MaDoanhNghiep" value="<%=MaDoanhNghiep %>"/></td>
                      </tr>
                      <tr>
                        <td>Hội đồng chấm điểm: </td>
                        <td>&nbsp;<%=Rs(1) %>
                          <input type="hidden" name="MaHoiDong" value="<% =Rs(0) %>" /></td>
                      </tr>
                      <tr>
                        <td>Sản phẩm chấm điểm: </td>
                        <td>&nbsp;<%=Rs(2)  %>
                          <input type="hidden" name="MaSanPham" value="<% =MaSanPham %>"/></td>
                      </tr>
<%
	if(Session("quyen")=2 and Session("ma_dn") >0) then
%>
						  <tr>
							<td colspan="2">Nhận xét: </td>
						  </tr>
						  <tr>
							<td colspan="2"><textarea name="txtContent" cols="65" rows="20"><% =NoiDung %></textarea></td>
						  </tr>
						  <tr>
							<td colspan="2"><input name="bntNhanXet" type="submit" id="bntNhanXet" value="Nhận xét" /></td>
						  </tr>
<%
	else
%>
						  ...
						  <tr>
							<td colspan="2"><textarea readonly="readonly" name="txtContent" cols="65" rows="20"><% =NoiDung %></textarea></td>
						  </tr>
<%
	end if
%>
