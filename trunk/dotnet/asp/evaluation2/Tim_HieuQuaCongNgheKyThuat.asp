...
function setValue(page) 
{
	...
	function move(){
		window.location = "Nhap_HieuQuaCongNgheKyThuat.asp"
	}
...
function istxtChiTietThongTinEmpty(formtimma) {
      if ( 	formtimma.timma.value == ""){
          alert("Bạn phải nhập tên hiệu quả công nghệ kỹ thuật cần tìm");
          return false;
       }
	   else {
          return true;
...
dim str
str = "Select TenHieuQuaCongNghe_KyThuat from HieuQuaCongNghe_KyThuat"
rstim.Open str,conn
%>
<form name="formtimma" method="post" action="Tim_HieuQuaCongNgheKyThuat.asp" onsubmit="return validateForm(this)">
<table>
  <tr align="center">
  	<td>Nhập tên hiệu quả công nghệ kỹ thuật cần tìm</td>	
...
<%
dim sqlStr
sqlStr ="select * from HieuQuaCongNghe_KyThuat inner join SanPham on SanPham.MaSanPham = HieuQuaCongNghe_KyThuat.MaSanPham inner join HoiDong on HoiDong.MaHoiDong = HieuQuaCongNghe_KyThuat.MaHoiDong where TenHieuQuaCongNghe_KyThuat = N'"+ Request.Form("timma")+ "'"
	
	RS.Open sqlStr, conn
		
			%>			<h3 align="center">Kết quả tìm kiếm hiệu quả công nghệ kỹ thuật</h3><table width="100%" border="1" cellpadding="5" align="center">
			    <tr align="center" >
				  
				  <td align="center"><b>Tên hiệu quả công nghệ kỹ thuật</b></td>
			...
			%>
				  <tr>
					  ...
					  <td align="center"><%=RS("TenHieuQuaCongNghe_KyThuat")%></td>
					  <td align="center"><%=RS("TenSanPham")%></td>
					  <td align="center"><%=RS("TenHoiDong")%></td>
					 
					  <td align="center"><%=RS("GhiChu")%></td>
	  <td><a href="Sua_HieuQuaCongNgheKyThuat.asp?MaHieuQuaCongNghe_KyThuat=<%=RS("MaHieuQuaCongNghe_KyThuat")%>">Sửa</a></td>
	  <td><a href="javascript:del(<%=RS("MaHieuQuaCongNghe_KyThuat")%>)">Xóa</a></td>
				  </tr>
			  <%	
			RS.MoveNext
			wend
...
<form action="Xoa_HieuQuaCongNgheKyThuat.asp" method="get" name="delFrm" id="delFrm">
