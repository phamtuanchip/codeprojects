          ...
          <td width="79%"><table width="100%" height="100%" border="0" cellpadding="00" cellspacing="0">
            <tr>
              ...
              <td width="96%" valign="top"><!-- InstanceBeginEditable name="nhom_thietke" -->
                    <div align="left">				    
                      <p align="center" class="tex_de_muc"><strong>Các phiếu đã nhập</strong></br>
                    </div>
                    <hr>
<%
	var i;
	var MaNhaCungCap = Request.QueryString("MaNhaCungCap");
	if(MaNhaCungCap =="all"){
%>
  <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#F3F3F3" bgcolor="#F7F7F7">
  <tr>
    <td width="69" bgcolor="#E4E4E4"><div align="center">Số thứ tự</div></td>
    <td width="141" bgcolor="#E4E4E4"><div align="center">Mã số phiếu</div></td>
	 <td width="242" bgcolor="#E4E4E4"><div align="center">Nh&agrave; cung cấp</div></td>
	 <td width="114" bgcolor="#E4E4E4"><div align="center">Ngày nhập</div></td>
	 </tr>
<%
	RS = oConn.Execute("SELECT ma_phieu,ten_nha_cc,ma_nha_cc,ngay_nhap FROM PHIEU");
%>
<%
i=1;
while(!RS.EOF){
...
%>	
  <tr>
  	<%
		P = oConn.Execute("select datepart(dd,ngay_nhap) ngay,datepart(mm,ngay_nhap) thang,datepart(yyyy,ngay_nhap) nam from PHIEU where ma_phieu ="+MaPhieu)
	%>
    <td bgcolor="#F7F7F7"><%=i++%></td>
    <td bgcolor="#F7F7F7"><div align="center"><a href="VOrderDetai.asp?MaPhieu=<%=RS("ma_phieu")%>">N<%=RS("ma_phieu")%></a></div></td>
	<td bgcolor="#F7F7F7"><a href="VOrder.asp?MaNhaCungCap=<%=RS("ma_nha_cc")%>"><%=RS("ten_nha_cc")%></a></td>
	<td bgcolor="#F7F7F7"><div align="center"><%=P("ngay")+"/"+P("thang")+"/"+P("nam")%></div></td>
	</tr>
<%RS.MoveNext}%>  
...
<%
	RS = oConn.Execute("SELECT ma_phieu,ten_nha_cc,ma_nha_cc,ngay_nhap FROM PHIEU WHERE ma_nha_cc ='"+MaNhaCungCap+"'");
%>
<%
i=1;
while(!RS.EOF){
...
<%}%>
<div align="right"><br>
Tổng cộng có : <%=i-1%> Phiếu nhập<br><hr>
</div>       
<%RS.Close;%>       
<!-- InstanceEndEditable --></td>
              </tr>
