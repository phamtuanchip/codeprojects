    ...
    if (val) { nm=val.name; if ((val=val.value)!="") {
      if (test.indexOf('isEmail')!=-1) { p=val.indexOf('@');
        ...
        if (test.indexOf('inRange') != -1) { p=test.indexOf(':');
          ...
          if (num<min || max<num) errors+='- '+nm+' Không được lớn hơn '+min+' .\n';
    } } } else if (test.charAt(0) == 'R') errors += '- '+nm+' is required.\n'; }
  } if (errors) alert('Bạn gặp phải lỗi yêu cầu nhập lại:\n'+errors);
...
//-->
				 ...
				 %>
                  ...
                  <p>&nbsp;</p>
                  <p>Nhập hệ số alpha O của sản phẩm:</p>
                  <%
				   if len(Request.Form()) >0 then
					...
					sqlupdate= "update chitiettochuc set alphao="& Request.Form(namealpha)&" where  machitiettochuc="&Request.Form(namebuocgc)&""
                  ...
                  <p><%=rs("tensanpham")%>&nbsp; </p>
                  <%rs.close()%>
                  <form action="Nhap_alphao.asp" method="post" name="chitietthongtin" id="chitietthongtin" onsubmit="MM_validateForm('Tổng alpha O','','RinRange1:1');return document.MM_returnValue" >
                    <table width="100%" border="1">
                      <tr>
                        <td>&nbsp;</td>
                        <td>Hệ số anpha O </td>
                      </tr>
                      <% rs.open "select * from chitiettochuc where masanpham = "& Session("ma_sp"),conn%>
                      <% dem=1
					  	while not rs.eof 
					  %>
                      <tr>
                        <td><%=rs("tenchitiettochuc")%>&nbsp;
                            <input name="machitiettt<%=dem%>" type="hidden" id="machitiettt<%=dem%>"  value="<%=rs("machitiettochuc")%>"/></td>
                        <td><input name="alphai<%=dem%>" type="text" id="alphai<%=dem%>" value="<%=rs("alphao")%>" size="3"/></td>
                      </tr>
                      <%dem = dem +1%>
                      <%rs.movenext()%>
                      <%wend %>
                      ...
                      <tr>
                        <td><div align="right">
                            <input name="Submit" type="submit"  onmouseover="document.getElementById('Tổng alpha O').value=kiemtra(<%=dem-1%>)" value="  Nhập  "/>
                        </div></td>
                        <td><input name="Tổng alpha O" type="text" id="Tổng alpha O"  size="3" readonly="true" />
