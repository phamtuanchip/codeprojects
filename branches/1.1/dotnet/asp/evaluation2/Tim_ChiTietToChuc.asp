...
function setValue(page) 
{
	...
	function move(){
		window.location = "Nhap_ChiTietToChuc.asp"
	}
...
function istxtChiTietThongTinEmpty(formtimma) {
      if ( 	formtimma.timma.value == ""){
          alert("Bạn phải nhập tên chi tiết tổ chức cần tìm");
          return false;
       }
	   else {
          return true;
...
dim str
str = "Select MaChiTietToChuc,TenChiTietToChuc from ChiTietToChuc"
rstim.Open str,conn
%>
<form name="formtimma" method="post" action="Tim_ChiTietToChuc.asp" onsubmit="return validateForm(this)">
<table>
  <tr align="center">
  	<td>Nhập tên chi tiết tổ chức cần tìm</td>	
...
<%
dim sqlStr
sqlStr ="select * from ChiTietToChuc inner join SanPham on SanPham.MaSanPham = ChiTietToChuc.MaSanPham inner join HoiDong on HoiDong.MaHoiDong = ChiTietToChuc.MaHoiDong where TenChiTietToChuc = N'"+ Request.Form("timma")+ "'"
	
	RS.Open sqlStr, conn
		
			%>			<h3 align="center">Kết quả tìm kiếm chi tiết tổ chức</h3><table width="100%" border="1" cellpadding="5" align="center">
			    <tr align="center" >
				 
				  <td align="center"><b>Tên chi tiết tổ chức</b></td>
			...
			%>
				  <tr>
					  ...
					  <td align="center"><%=RS("TenChiTietToChuc")%></td>
					  <td align="center"><%=RS("TenSanPham")%></td>
					  <td align="center"><%=RS("TenHoiDong")%></td>
					  <td align="center"><%=RS("GhiChu")%></td>
	  <td><a href="Sua_ChiTietToChuc.asp?MaChiTietToChuc=<%=RS("MaChiTietToChuc")%>">Sửa</a></td>
	  <td><a href="javascript:del(<%=RS("MaChiTietToChuc")%>)">Xóa</a></td>
				  </tr>
			  <%	
			RS.MoveNext
			wend
...
<form action="Xoa_ChiTietToChuc.asp" method="get" name="delFrm" id="delFrm">
