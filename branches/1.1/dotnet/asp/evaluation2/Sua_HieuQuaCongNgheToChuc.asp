...
<title>Sửa hiệu quả công nghệ tổ chức</title>
<!-- InstanceEndEditable -->
...
Dim RsSua
Dim str
str = "select * from HieuQuaCongNghe_ToChuc inner join HoiDong on HieuQuaCongNghe_ToChuc.MaHoiDong = HoiDong.MaHoiDong inner join SanPham on HieuQuaCongNghe_ToChuc.MaSanPham = SanPham.MaSanPham where HieuQuaCongNghe_ToChuc.MaHieuQuaCongNghe_ToChuc = '"+ Request.QueryString("MaHieuQuaCongNghe_ToChuc")+ "'"
set RsSua = Server.CreateObject("ADODB.Recordset")
RsSua.Open str,conn
%>
<center><font face="Arial, Helvetica, sans-serif" color="#006633" size="4">Sửa hiệu quả công nghệ tổ chức</font>
<p>
Chú ý : Bạn không thể sửa những trường khóa có dấu (*)
</center>
<form name="formsuahieuquacongnghetochuc" id="formsuahieuquacongnghetochuc" method="post" action="Save_HieuQuaCongNgheToChuc.asp" onSubmit="return validateForm(this)">
  <table width="418" border="0" align="center">
    <tr>
      <td width="172"></td>
      <td width="230"><input name="mahqcntc" type="hidden" id="mahqcntc" value="<%=RsSua("MaHieuQuaCongNghe_ToChuc")%>" readonly>&nbsp;&nbsp;</td>
    </tr>
    <tr>
      <td>Tên hiệu quả công nghệ tổ chức</td>
      <td><input name="tenhqcntc" type="text" id="tenhqcntc" value="<%=RsSua("TenHieuQuaCongNghe_ToChuc")%>" size="30"></td>
    </tr>
    <tr>
      <td>Tên sản phẩm</td>
