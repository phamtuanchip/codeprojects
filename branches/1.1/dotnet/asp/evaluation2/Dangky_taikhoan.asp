  ...
  for (i=0; i<(args.length-2); i+=3) { test=args[i+2]; val=MM_findObj(args[i]);
    if (val) { nm=val.name; if ((val=val.value)!="") {
      if (test.indexOf('isEmail')!=-1) { p=val.indexOf('@');
        if (p<1 || p==(val.length-1)) errors+='- '+nm+' phải đúng định dạng.\n';
      } else if (test!='R') { num = parseFloat(val);
        if (isNaN(val)) errors+='- '+nm+' phải là số.\n';
        if (test.indexOf('inRange') != -1) { p=test.indexOf(':');
          min=test.substring(8,p); max=test.substring(p+1);
          if (num<min || max<num) errors+='- '+nm+' phải nằm trong khoảng '+min+' and '+max+'.\n';
        ...
        <tbody>
          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <% if (len(Request.Form("Họ và tên"))<1)then %>
                  <p>Đăng ký tài khoản để xem thông tin </p>
                  <form action="Dangky_taikhoan.asp" method="post" name="dangkyten" id="dangkyten" onsubmit="MM_validateForm('Họ và tên','','R','Năm thành lập','','NisNum','Thư điện tử','','RisEmail');return document.MM_returnValue">
                    <table width="100%" border="0" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="40%">Tên của bạn </td>
                        <td width="60%"><input name="Họ và tên" type="text" id="Họ và tên" size="35"/></td>
                      </tr>
                      <tr>
                        <td>Tên công ty </td>
                        <td><input name="tencongty" type="text" id="tencongty" size="35"/></td>
                      </tr>
                      <tr>
                        <td>Lĩnh vực ngành nghề kinh doanh </td>
                        <td><textarea name="linhvuckinhdoanh" cols="30" rows="4" id="linhvuckinhdoanh">               </textarea></td>
                      </tr>
                      <tr>
                        <td>Năm thành lập của công ty </td>
                        <td><input name="Năm thành lập" type="text" id="Năm thành lập" size="35"/></td>
                      </tr>
                      <tr>
                        <td>Trang chủ của công ty </td>
                        <td><input name="trangchu" type="text" id="trangchu" size="35"/></td>
                      </tr>
                      <tr>
                        <td colspan="2" align="center"><strong><br />
Chúng tôi có thể liên lạc với bạn qua </strong><br />
<br />
</td>
                        </tr>
                      <tr>
                        <td>Thư điện tử </td>
                        <td><input name="Thư điện tử" type="text" id="Thư điện tử" size="35"/></td>
                      </tr>
                      <tr>
                        <td>Điện thoại </td>
                        <td><input name="dienthoai" type="text" id="dienthoai" size="35"/></td>
                      </tr>
                      <tr>
                        <td>Địa chỉ </td>
                        <td><textarea name="diachi" cols="30" rows="4" id="diachi"></textarea></td>
                      </tr>
                      <tr>
                        <td>&nbsp;</td>
                  ...
                  <% 
				   ...
				   sqlinsert ="insert into [guest] (hovaten,tencongty,linhvuckinhdoanh,namthanhlap,trangchu,hopthu,dienthoai,diachi) values(N'"
				   sqlinsert = sqlinsert & Request.Form("Họ và tên")&"', N'"	
				   sqlinsert = sqlinsert & Request.Form("tencongty")&"', N'"
				   sqlinsert = sqlinsert & Request.Form("linhvuckinhdoanh")&"', N'"					   			   	   
				   sqlinsert = sqlinsert & Request.Form("Năm thành lập")&"', N'"
				   sqlinsert = sqlinsert & Request.Form("trangchu")&"', N'"
				   
				   sqlinsert = sqlinsert & Request.Form("Thư điện tử")&"', N'"
				   sqlinsert = sqlinsert & Request.Form("dienthoai")&"', N'"
				   sqlinsert = sqlinsert & Request.Form("diachi")&"')"
				   conn.execute sqlinsert
				   
				    'Kiểm tra sự tồn tại của File
						dim efs , fs,f,strPath, FName
						strPath = server.MapPath("doanhnghiep")
						 rs.open "select max (idguest) as idmax from [guest] ",conn
						 FName = rs("idmax")
						set efs=Server.CreateObject("Scripting.FileSystemObject")
						if efs.FolderExists(strPath & "\" & FName)=true then
							
							set efs=nothing
						else
					    set fs=Server.CreateObject("Scripting.FileSystemObject")
						set f=fs.CreateFolder(strPath & "\" & FName)
						set f=nothing
						set fs=nothing
...
%>
                  <%'=sqlinsert%>
Cảm ơn bạn đã đăng ký, bạn hãy kiểm tra thư điện tử để được cấp tài khoản đăng nhập
<table align="center" border="1" width="100%" height="50%" bordercolor="#006633" bordercolorlight="#0000CC"  cellpadding="8">
  <tr>
    <td><p>Nếu bạn muốn đưa logo của công ty mình lên , xin mời làm ở đây </p>
...
<script language="javascript">
	function checkFile()
	{
		if(form.File1.value=="")
		{
			alert("Bạn phải chọn file ảnh để đưa lên !");
			return false;
		}
...
<% 'Response.Write(strPath & "\" & FName) %>
		<form action="uploadlogo.asp?idcp=<%=FName%>" method="post" enctype="multipart/form-data" name="form" id="form">
          <input type="file" name="File1" size="30" />
          <input name="submit" type="submit" onclick="return checkFile();" value="Upload" />
          <input type="button" name="Button" value="Cancel" onclick="tranchu.asp" />
      </form></td>
  </tr>
</table>
<% end if %>
