...
<title>Cập nhật điểm sản phẩm theo công đoạn sản xuất</title>
<!-- InstanceEndEditable -->
...
<%
	...
	for i=0 to CongDoanSize-1
		...
		for j=0 to HieuQuaSize-1
			...
			HCDmax = Request.Form("Pmax"&i&j)
				...
				sql = "SELECT COUNT(MaThanhVien) FROM DiemChiTieu_KyThuat WHERE MaBuocGiaCong="+MaHieuQua+" AND MaThanhVien="+MaThanhVien+" AND MaChiTieu="+MaGiaCong'MaHieuQua
				RS2.Open sql,conn
				Response.Write(sql)
				Response.Write("<Br>")
				Response.Write("<Br>")
				Response.Write("<Br>")
				if(RS2(0)>0) then
						UpdateSql="Update DiemChiTieu_KyThuat set Tamin=" & HCDmin & ", Tamax=" & HCDmax & "WHERE MaBuocGiaCong="+MaHieuQua+" AND MaThanhVien="+MaThanhVien+" AND MaChiTieu="+MaGiaCong
						conn.execute UpdateSql
				'		Response.Write(UpdateSql)
				'		Response.Write("<Br>")
					outStr = "Chúc mừng bạn đã sửa điểm thành công."
				else
				sql = "Insert into DiemChiTieu_KyThuat (MaBuocGiaCong, MaThanhVien, MaChiTieu, Tamin, Tamax) VALUES ("+MaHieuQua+","+MaThanhVien+","+MaGiaCong+","+HCDmin+","+HCDmax+")"
				'Response.Write(sql)
				'Response.Write("<Br>")
					conn.execute sql
	...
	Response.Redirect("PhieuDiemSanPhamTheoCongDoanSanXuat.asp?outStr="&outStr&"&ma_tv="&MaThanhVien)
%>
