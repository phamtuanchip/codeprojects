...
<script language="JavaScript" type="text/javascript">
	function check(){
	document.all.formdiemtieuchuanmoitruong.lamda.focus();
}
function validateForm(f){

	if(!istxtLamDaEmpty(f))
		return false;
	if(!isCheckNumber(f))
		 return false;
	if(!istxtLamdaEmpty(f))
		 return false;
	if(!istxtBuocGiaCongEmpty(f))
		 return false;
	if(formdiemtieuchuanmoitruong.lamda >= 0.9 || formdiemtieuchuanmoitruong.lamda <=0.1 )
	{
		alert("Lamda phải nằm trong khoảng từ 0.1 đến 0.9.");
		return false;
	}
}
function istxtLamDaEmpty(formdiemtieuchuanmoitruong) {
    var str = document.formdiemtieuchuanmoitruong.lamda.value;
    var re = /^[-]?\d*\.?\d*$/;
	str = str.toString();
    if ( formdiemtieuchuanmoitruong.lamda.value == "" || 
			formdiemtieuchuanmoitruong.lamda.value  ==null){
          alert("Bạn phải nhập LamDa");
          return false;
       }
	if (!str.match(re)) {
        alert("LamDa phải là kiểu số");
       
    } else 
	  return true;
}
function isCheckNumber(formdiemtieuchuanmoitruong) {
    var str = formdiemtieuchuanmoitruong.lamda.value;
    if (parseFloat(str)>=0.1 && parseFloat(str)<=0.9)
	{
		return true;
	}
	else{
		alert("Lamda phải nhập trong khoảng từ 0.1 đến 0.9");
		return false;
	}
}
function istxtLamdaEmpty(formdiemtieuchuanmoitruong) {
      if ( 	formdiemtieuchuanmoitruong.slMoiTruong.value == "0"){
          alert("Bạn phải chọn tên nhân tố môi trường");
          return false;
       }
	   else {
          return true;
...
function istxtBuocGiaCongEmpty(formdiemtieuchuanmoitruong) {
      if ( 	formdiemtieuchuanmoitruong.slGiaCong.value == "0"){
          alert("Bạn phải chọn tên bước gia công");
          return false;
       }
	   else {
          return true;
...
<%
	...
	strSqlGiaCong = "Select * FROM NhapDienTCMTView1 WHERE MaDoanhNghiep="&session("ma_dn")
	strSqlMoiTruong = "Select * FROM NhapDienTCMTView12 WHERE MaDoanhNghiep="&session("ma_dn")
	RsGiaCong.Open strSqlGiaCong,conn
	RsMoiTruong.Open strSqlMoiTruong,conn
	set tmpGiaCong = Request.QueryString("MaBuocGiaCong")
	set tmpMoiTruong = Request.QueryString("MaNhanToMoiTruong")
%>
<center><font face="Arial, Helvetica, sans-serif" color="#006633" size="4">Nhập điểm tiêu chuẩn môi trường</font></center><P>
<table align="center">
<tr>
	<td>Nếu như tên nhân tố môi trường ở dưới trống xin nhấn vào nút Tạo nhân tố môi trường</td>
</tr>
<tr>
	<td align="center">Nếu muốn xem danh sách Điểm tiêu chuẩn môi trường xin hãy nhấn <a href="DanhSach_DiemTieuChuanMoiTruong.asp">vào đây</a></td>
</tr>
</table>
<form action="XuLy_NhapDiemTieuChuanMoiTruong.asp" method="post" name="formdiemtieuchuanmoitruong" id="formdiemtieuchuanmoitruong" onsubmit="return validateForm(this)">
  <table width="460" border="0" align="center">
    <tr>
      <td width="144">Tên nhân tố môi trường</td>
      <td width="306"><select name="slMoiTruong">
	  <option value="0">----- Chọn tên nhân tố môi trường -----</option>
		<%
			do while (not RsMoiTruong.EOF)
		%>
		
		<option value="<%=RsMoiTruong("MaNhanToMoiTruong")%>"><%=RsMoiTruong("TenNhanToMoiTruong")%></option>
		<%
			RsMoiTruong.MoveNext
			loop
		%>
	</select>
        </td>
    ...
    <tr>
      <td>Tên bước gia công </td>
      <td> <select name="slGiaCong">
	  <option value="0">----- Chọn tên bước gia công -----</option>
		<%
			do while (not RsGiaCong.EOF)
		%>
	
		<option value="<%=RsGiaCong("MaBuocGiaCong")%>"><%=RsGiaCong("TenBuocGiaCong")%></option>
		<%
			RsGiaCong.MoveNext
			loop
		%>
    ...
    <tr>
      <td>Lamda</td>
      <td><input type="text" name="lamda" title="Lamda phải từ 0.1 đến 0.9" /></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>        <input name="chapnhan" type="submit" id="chapnhan" value="Chấp nhận" />
      <input name="huybo" type="reset" id="huybo" value="Hủy bỏ" /></td>
    </tr>
  </table>
  <hr />
  <table align="right">
  <tr>
  <td><input type="button" name="taomt" value="Tạo nhân tố môi trường" onclick="movemt();"/></td>
  </tr>
  </table>
</form>
