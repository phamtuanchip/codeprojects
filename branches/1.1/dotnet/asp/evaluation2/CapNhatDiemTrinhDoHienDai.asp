...
<title>Cập nhật điểm trình độ hiện đại</title>
<!-- InstanceEndEditable -->
          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
				...
				<!--#include file="connection.inc" -->
				  ...
				  On error resume next
				  		Tcount = Request.QueryString("TminCount")
						Hcount=Request.QueryString("HminCount")
						Icount=Request.QueryString("IminCount")
						Ocount=Request.QueryString("OminCount")
						i=0
						Tcount=Cint(Tcount)-1
						MaThanhVien=Request.QueryString("MaThanhVienSelect")
						MaGiaCongTemp=Request.QueryString("TminID0")
						'Rs.open ("Select count(*) From DiemBuocGiaCong where MaThanhVien=" + MaThanhVien + " and MaBuocGiaCong=" + MaGiaCongTemp),conn
			'if Rs(0)>0 then 
						'	Response.Write("Thành viên này đã chấm điểm rồi. Bãn hãy chọn thành viên khác.")
			'else
						for i=0 to Tcount
							maRequestT=Request.QueryString("TminID"&i)
							Tmin=Request.QueryString("Tmin"&i)
							Tmax=Request.QueryString("Tmax"&i)
							Rs.open "Select count(*) from DiemBuocGiaCong where MaThanhVien=" & MaThanhVien & " and MaBuocGiaCong=" & maRequestT,conn
							if Rs(0)>0 then
								SqlUpdate="Update DiemBuocGiaCong set Tmax=" & Tmax & ", Tmin=" & Tmin & " where MaThanhVien=" & MaThanhVien & " and MaBuocGiaCong=" & maRequestT
								conn.execute SqlUpdate
								'Response.Write(SqlUpdate)
								'Response.Write("<br>")
							else
								sql="insert into DiemBuocGiaCong(MaThanhVien,MaBuocGiaCong,Tmin,Tmax) Values (" + MaThanhVien + "," + maRequestT + "," + Tmin + "," + Tmax + ")"
							conn.Execute(sql)
							'Response.Write(sql)
							'Response.Write("<br>")
							end if
...
'						Response.Write("<Br>")
						...
						for i=0 to Hcount
							maRequestH=Request.QueryString("HminID"&i)
							Hmin=Request.QueryString("Hmin"&i)
							Hmax=Request.QueryString("Hmax"&i)
							
							Rs.open "Select count(*) from DiemTieuChuanConNguoi where MaThanhVien=" & MaThanhVien & " and MaNhanToConNguoi=" & maRequestH,conn
							if Rs(0)>0 then
								SqlUpdate="Update DiemTieuChuanConNguoi set Hmax=" & Hmax & ", Hmin=" & Hmin & " where MaThanhVien=" & MaThanhVien & " and MaNhanToConNguoi=" & maRequestH
								conn.execute SqlUpdate
								'Response.Write(SqlUpdate)
								'Response.Write("<br>")
							else
								sql="Insert into DiemTieuChuanConNguoi(MaThanhVien,MaNhanToConNguoi,Hmax,Hmin) Values ("+ MaThanhVien + ","+ maRequestH +","+ Hmax +"," + Hmin  +")"
...
'				'			maGC=(+0)
						...
						for i=0 to Icount
							maRequestI=Request.QueryString("IminID"&i)
							Imin=Request.QueryString("Imin"&i)
							Imax=Request.QueryString("Imax"&i)
							Rs.open "Select count(*) from DiemTieuChuanThongTin where MaThanhVien=" & MaThanhVien & " and MaChiTietThongTin=" & maRequestI,conn
							if Rs(0)>0 then
								SqlUpdate="Update DiemTieuChuanThongTin set Imax=" & Imax & ", Imin=" & Imin & " where MaThanhVien=" & MaThanhVien & " and MaChiTietThongTin=" & maRequestI
								conn.execute SqlUpdate
								'Response.Write(SqlUpdate)
								'Response.Write("<br>")
							else
								sql="Insert into DiemTieuChuanThongTin(MaThanhVien,MaChiTietThongTin,Imax,Imin) Values ("+ MaThanhVien + ","+  maRequestI +","+ Imax +"," + Imin  +")"
...
'				'			maGC=(+0)
						...
						Ocount=Cint(Ocount)-1
						for i=0 to Ocount
							maRequestO=Request.QueryString("OminID"&i)
							Omin=Request.QueryString("Omin"&i)
							Omax=Request.QueryString("Omax"&i)
							Rs.open "Select count(*) from DiemTieuChuanToChuc where MaThanhVien=" & MaThanhVien & " and MaChiTietToChuc=" & maRequestO,conn
							if Rs(0)>0 then
								SqlUpdate="Update DiemTieuChuanToChuc set Omax=" & Omax & ", Omin=" & Omin & " where MaThanhVien=" & MaThanhVien & " and MaChiTietToChuc=" & maRequestO
								conn.execute SqlUpdate
								'Response.Write(SqlUpdate)
								'Response.Write("<br>")
								outStr = "Chúc mừng bạn đã sửa điểm thành công."
							else
								sql="Insert into DiemTieuChuanToChuc(MaThanhVien,MaChiTietToChuc,Omax,Omin) Values ("+ MaThanhVien + ","+ maRequestO  +","+ Omax +"," +  Omin +")"
...
'				'			maGC=(+0)
	...
	Response.Redirect("PhieuChoDiemTrinhDo.asp?outStr="&outStr&"&ma_tv="&MaThanhVien)
				'end if		
						'Response.Write(Tcount + "; " + Hcount + "; " + Icount + "; " + Ocount)
