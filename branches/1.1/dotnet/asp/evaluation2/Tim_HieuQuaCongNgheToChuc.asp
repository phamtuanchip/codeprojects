...
function setValue(page) 
{
	...
	function move(){
		window.location = "Nhap_HieuQuaCongNgheToChuc.asp"
	}
...
function istxtChiTietThongTinEmpty(formtimma) {
      if ( 	formtimma.timma.value == ""){
          alert("Bạn phải nhập tên hiệu quả công nghệ tổ chức cần tìm");
          return false;
       }
	   else {
          return true;
...
dim str
str = "Select TenHieuQuaCongNghe_ToChuc from HieuQuaCongNghe_ToChuc"
rstim.Open str,conn
%>
<form name="formtimma" method="post" action="Tim_HieuQuaCongNgheToChuc.asp" onsubmit="return validateForm(this)">
<table>
  <tr align="center">
  	<td>Tìm theo mã hiệu quả công nghệ tổ chức</td>	
...
<%
dim sqlStr
sqlStr ="select * from HieuQuaCongNghe_ToChuc inner join SanPham on SanPham.MaSanPham = HieuQuaCongNghe_ToChuc.MaSanPham inner join HoiDong on HoiDong.MaHoiDong = HieuQuaCongNghe_ToChuc.MaHoiDong where TenHieuQuaCongNghe_ToChuc = N'"+ Request.Form("timma")+ "'"
	
	RS.Open sqlStr, conn
		
			%>			<h3 align="center">Kết quả tìm kiếm hiệu quả công nghệ tổ chức</h3><table width="100%" border="1" cellpadding="5" align="center">
			    <tr align="center" >
				  
				  <td align="center"><b>Tên hiệu quả công nghệ tổ chức</b></td>
			...
			%>
				  <tr>
					  ...
					  <td align="center"><%=RS("TenHieuQuaCongNghe_ToChuc")%></td>
					  <td align="center"><%=RS("TenSanPham")%></td>
					  <td align="center"><%=RS("TenHoiDong")%></td>
					  <td align="center"><%=RS("GhiChu")%></td>
	  <td><a href="Sua_HieuQuaCongNgheToChuc.asp?MaHieuQuaCongNghe_ToChuc=<%=RS("MaHieuQuaCongNghe_ToChuc")%>">Sửa</a></td>
	  <td><a href="javascript:del(<%=RS("MaHieuQuaCongNghe_ToChuc")%>)">Xóa</a></td>
				  </tr>
			  <%	
			RS.MoveNext
			wend
...
<form action="Xoa_HieuQuaCongNgheToChuc.asp" method="get" name="delFrm" id="delFrm">
