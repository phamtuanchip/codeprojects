          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <% 
				   ...
				   rs.open "select * from [users] where tentruycap='"&ten&"' and matkhau= '"&matkhau&"'" ,conn
				   if rs.eof then  %> 
				   <b> Tên đăng nhập hoặc mật khẩu không đúng! <br />
				   Yêu cầu đăng nhập lại, nếu bạn quên mật khẩu hãy liên hệ với nhà quản trị hệ thống</b> 
				   <%
				   'session(trangthai)=false
				   else 
				   session("trangthai")=true
				   session("ten")=rs("tentruycap")
				   session("quyen")=rs("quyen")
				   session("ma_doanhnghiep")=rs("madoanhnghiep")
				   Session("ma_dn")=rs("madoanhnghiep")				   
				   select case session("quyen")
				   		case 1 : 
								  if (rs("madoanhnghiep")>0) then
								  sql ="select count(tendoanhnghiep) dem from doanhnghiep where madoanhnghiep= "&rs("madoanhnghiep")&" and status=1 "
								  dim rscount
								  set rscount = server.CreateObject("adodb.recordset")
								  rscount.open sql,conn
								  if rscount(0)>0 then
								  ...
								  rscount.clsoe
									...
									Response.Redirect("quantri.asp") 
								 end if
						case 2 : Response.Redirect("nguoidungdangnhap.asp")
						case 3 : Response.Redirect("trangchu.asp")
