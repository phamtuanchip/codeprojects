...
<title>Chọn danh mục chấm điểm</title>
<!-- InstanceEndEditable -->
          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  <script language="javascript" >
				  	function kiemtra()
				  {
				  	if  ((chondnhdsp.chondn.value==0) || (chondnhdsp.chonhd.value==0) || (chondnhdsp.chonsp.value==0) ) {
					alert("Bạn phải chọn đầy đủ")
					return false;
				  };
					//return false;
				  }
                  ...
                  <p>
                    <% if (cint((session("ma_dn")))>0) then ' kiem tra xem session (ma_doanhnghiep) co null hay khong%>
                    Chào mừng đến với phần nhập điểm cho doanh nghiệp</p>
                  <fieldset >
				 ...
				 %>
                  ...
                  <%end if ' ket thuc phan ma doanh nghiep 1 %>
                  </fieldset>
                  <p>&nbsp;                  </p>
                  <form id="chondnhdsp" name="chondnhdsp" method="post" action="DanhSachPhieuDiem.asp" onsubmit="return kiemtra();">
  <table width="100%" border="1" align="center">
    <tr>
      <td width="30%">&nbsp;</td>
      <td width="70%">
	  <%rs.open "select * from doanhnghiep where madoanhnghiep="& Session("ma_doanhnghiep")& "and status =1",conn%>
	  <input name="chondn" type="hidden" id="chondn"  value="<%=Session("ma_doanhnghiep")%>"/>
	  <%rs.close()%>	  </td>
    </tr>
    <tr>
      <td>Chọn hội đồng</td>
      <td><select name="chonhd" id="chonhd" onchange="window.location='nguoidungdangnhap.asp?ma_hd='+this.value">
        <%rs.open "select * from hoidong where madoanhnghiep="& Session("ma_doanhnghiep")& "and status =1",conn%>
    ...
    <tr>
      <td>Chọn sản phẩm</td>
      <td><select name="chonsp" id="chonsp" onchange="window.location='nguoidungdangnhap.asp?ma_hd='+chondnhdsp.chonhd.value+'&amp;ma_sp='+this.value;">
        <%if (Request.QueryString("ma_hd")>0)  then  %>
        <%rs.open "select * from sanpham where madoanhnghiep="& Session("ma_doanhnghiep")& " and mahoidong="& Request.QueryString("ma_hd")&" and status=1" ,conn%>
        <%else%>
        <%rs.open "select * from sanpham where madoanhnghiep="& Session("ma_doanhnghiep"),conn%>
        <%end if%>
    ...
    <tr>
      <td>&nbsp;</td>
      <td>
	     <input type="submit" name="Submit" value="Nhập điểm" />
        <input type="reset" name="Submit2" value="Làm lại" />	</td>
    </tr>
  </table>
</form>

				  <%end if ' ket thuc kiem tra session ("ma_doanhnghiep")%>
