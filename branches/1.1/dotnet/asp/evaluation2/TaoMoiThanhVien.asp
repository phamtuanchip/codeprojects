...
//-->
                ...
                <h3 align="center">Tạo mới Thành Viên </h3>
                <fieldset >
                ...
                <p align="center">&nbsp;</p>
                <p align="center">Mời bạn điền đầy đủ thông tin và ấn vào nút Tạo mới để hoàn tất việc tạo mới Thành Viên hoặc ấn vào nút Bỏ qua để xóa các thông tin đã điền và nhập lại.</p>
<%
Dim errorStr
...
RS.Open "SELECT COUNT(MaHoiDong) FROM HoiDong", conn
if(RS(0) < 1) then
	errorStr="Chưa tồn tại một Hội Đồng nào,xin bạn vui lòng tạo mới một Hội Đồng rồi mới có thể tạo mới thành viên."
RS.Close
	Response.Redirect("TaoMoiHoiDong.asp?errorStr="+errorStr)
else
	RS.Close
end if
%>
                <form action="Tao_Moi.asp" method="post" name="form1" id="form1" onsubmit="MM_validateForm('TenThanhVien','','R');return document.MM_returnValue">
	<table width="95%" border="0" align="center">
	<tr>
		<td width="50%" align="right">T&ecirc;n thành vi&ecirc;n</td>
		<td width="50%"><input size="45" type="text" name="TenThanhVien" title="Tên thành viên" value="<%=Request.QueryString("TenThanhVien")%>" />
		<input type="hidden" style="display:none " name="loai" value="ThanhVien" />          </td>
	</tr>
	<tr>
		<td align="right">Chức danh </td>
		<td><select name="MaViTri" onchange="showIt(this.value)">
		<option value="0">Chọn một chức danh</option>
		<%
	...
	<tr>
		...
		<td><select name="MaHoiDong" onchange="showItHD(this.value)">
		<%
		if(Session("Ma_DoanhNghiep")=0) then
		sqlStr = "SELECT MaHoiDong, TenHoiDong FROM HoiDong WHERE Status = 1 ORDER BY TenHoiDong"
		
		else	
		sqlStr = "SELECT MaHoiDong, TenHoiDong FROM HoiDong WHERE Status = 1 AND MaDoanhNghiep = "&Session("Ma_DoanhNghiep")&" ORDER BY TenHoiDong"
		end if
		RS.Open sqlStr,conn
		while(not RS.EOF)
		%>
		...
		RS.Close
		%>
		<option value="0">Chưa ở trong Hội Đồng nào cả</option>
		<option value="-1">Thêm mới Hội Đồng...</option>
		</select>          </td>
	</tr>
	<%
	if(Session("Ma_DoanhNghiep")=0) then
	...
	<%end if%>
	<input type="text" name="TenDoanhNghiep" style="display:none" />		</td>
	</tr>
	<tr>
...
<table width="100%" border="1" cellpadding="5">
			...
			' rs.Open;
			%>     </p>     <form action="TaoMoiThanhVien.asp" method="get" name="viewFrm" id="viewFrm">
