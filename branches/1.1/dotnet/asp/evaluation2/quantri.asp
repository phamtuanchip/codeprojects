        ...
        <tbody>
          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  <p>Trang thống kê toàn bộ thông tin hệ thống </p>
				  <p>&nbsp;</p>
				  <!--#include file="connection.inc" -->
                   <%
						rs.open "select count(*) dem from doanhnghiep where status=1 ",conn
						 dn=rs(0)
						rs.close()
						rs.open "select count(*) dem from hoidong where status=1 ",conn
						 hd=rs(0)
						rs.close()
						rs.open "select count(*) dem from sanpham where status=1 ",conn
						 sp=rs(0)
						rs.close()
						rs.open "select count(*) dem from thanhvien where status=1 ",conn
						 tv=rs(0)
						rs.close()
						rs.open "select count(*) dem from [users] ",conn
						 nd=rs(0)
						rs.close()
						rs.open "select count(*) dem from [users] where quyen=1 ",conn
						 a=rs(0)
						rs.close()
						rs.open "select count(*) dem from [users] where quyen=2 ",conn
						 u=rs(0)
						rs.close()
						rs.open "select count(*) dem from [users] where quyen=3 ",conn
						 g=rs(0)
						rs.close()
						rs.open "select count(*) dem from [guest] where tentruycap is null ",conn
						 r=rs(0)
						rs.close()
				  ...
				  <table width="100%" border="1">
                    <tr>
                      <td width="217"><a href="XemDanhSach.asp?loai=DoanhNghiep" title="Bấm vào đây để xem danh sách các doanh nghiệp">Tổng số doanh nghiêp</a></td>
                      <td width="142"><%=dn%></td>
                    </tr>
                    <tr>
                      <td><a href="XemDanhSach.asp?loai=HoiDong" title="Bấm vào đây để xem danh sách các hội đồng">Tổng số hội đồng</a></td>
                      <td><%=hd%></td>
                    </tr>
                    <tr>
                      <td><a href="quantri_taikhoan.asp" title="Bấm vào đây để xem danh sách người dùng và tạo tài khoản mới">Tổng số tài khoản người dùng </a></td>
                      <td><%=nd%>: có <%=a%> quản trị,<%=u%> người dùng,<%=g%> khách</td>
                    </tr>
                    <tr>
                      <td><a href="capquyendangnhap.asp">Danh sách khách đang ký </a></td>
                      <td><%=g%> khách</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      <td>Số người đã đăng ký chưa được cấp tài khoản </td>
                      <td><%=r%> <%if r>0 then %>
                      <a href="capquyendangnhap.asp">Xem và cấp quyền đăng nhập </a> <%end if%></td>
                    </tr>
                  </table>
				  <p>&nbsp;</p>
