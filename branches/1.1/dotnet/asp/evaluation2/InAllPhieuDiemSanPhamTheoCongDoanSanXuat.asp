...
<p class="style2">
  ...
  <%
		...
		sql = "SELECT ThanhVien.MaThanhVien, ThanhVien.TenThanhVien FROM ThanhVien INNER JOIN HoiDongChamDiem ON ThanhVien.MaThanhVien = HoiDongChamDiem.MaThanhVien WHERE ThanhVien.Status=1 and MaHoiDong = "&MaHoiDong & " and HoiDongChamDiem.Status=1"
		RS2.Open sql,conn
...
<div align="center" class="style2 style4"><span class="style6"><br />
  ...
  <table width="100%" border="1">
    ...
    <tr>
      <td><div align="center"> 	
	  	<% if RsMaChiTieu.recordcount>0 then
			 RsMaChiTieu.moveFirst() 
			end if
		%>
        ...
        <tr rowspan="4" height="20">
          ...
          <tr height="20">
			...
			<%
		'
		sql5 = "SELECT Tamin,Tamax From DiemChiTieu_KyThuat where MaThanhVien=" & RS2(0) & " and MaBuocGiaCong=" & RsMaBuocGiaCong(0) & " and MaChiTieu=" & RsMaChiTieu(0)
		RsDiem.Open sql5,conn 
		'Response.Write(RS2(0) & "; " & RsMaBuocGiaCong(0) & "; " & RsMaChiTieu(0))
		'Response.Write(sql5)
		dim diemTaMin
		dim diemTaMax
		
		if (not RsDiem.eof) then 
		diemTaMin = RsDiem("Tamin")
		diemTaMax = RsDiem("Tamax")
		else
		diemTaMin = 0
		diemTaMax = 0
		end if
		...
		%>
              ...
              <input type="hidden" name="MaHieuQua<%=CongDoanSize%><%=HieuQuaSize%>" value="<%=RsMaBuocGiaCong(0)%>" />              </td>
					    <td>
					      <input type="hidden" name="Pmin<%=CongDoanSize%><%=HieuQuaSize%>" size="3" />		
		
						 <%=diemTaMin %> 			    </td>
					    <td>
					      <input type="hidden" name="Pmax<%=CongDoanSize%><%=HieuQuaSize%>" size="3" />			<%=diemTaMax %> 		    </td>
		    </tr>
            <%
