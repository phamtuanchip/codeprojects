...
function setValue(page) 
{
	...
	function move(){
		window.location = "Nhap_HieuQuaCongNgheThongTin.asp"
	}
...
<script language="JavaScript" type="text/javascript">
	function check(){
	document.all.formtimma.timma.focus();
}
function validateForm(f){

	if(!istxtChiTietThongTinEmpty(f))
		 return false
	if(!istxtThongTinEmpty(f))
		 return false
}
function istxtChiTietThongTinEmpty(formtimma) {
      if ( 	formtimma.timma.value == ""){
          alert("Bạn phải nhập tên hiệu quả công nghệ thông tin cần tìm");
          return false;
       }
	   else {
          return true;
...
<%
dim rstim
set rstim = Server.CreateObject("ADODB.Recordset")
dim str
str = "Select TenHieuQuaCongNghe_ThongTin from HieuQuaCongNghe_ThongTin"
rstim.Open str,conn
%>
<form name="formtimma" method="post" action="Tim_HieuQuaCongNgheThongTin.asp" onsubmit="return validateForm(this)">
<table>
  <tr align="center">
  	<td>Nhập mã hiệu quả công nghệ thông tin cần tìm</td>	
	<td><input type="text" name="timma" /></td>
...
<%
dim sqlStr
sqlStr ="select * from HieuQuaCongNghe_ThongTin inner join SanPham on SanPham.MaSanPham = HieuQuaCongNghe_ThongTin.MaSanPham inner join HoiDong on HoiDong.MaHoiDong = HieuQuaCongNghe_ThongTin.MaHoiDong where TenHieuQuaCongNghe_ThongTin = N'"+ Request.Form("timma")+ "'"
	
	RS.Open sqlStr, conn
		
			%>			<h3 align="center">Kết quả tìm kiếm hiệu quả công nghệ thông tin</h3><table width="100%" border="1" cellpadding="5" align="center">
			    <tr align="center" >
				  
				  <td align="center"><b>Tên hiệu quả công nghệ thông tin</b></td>
				  <td align="center"><b>Tên sản phẩm</b></td>
				  <td align="center"><b>Tên hội đồng</b></td>
			...
			%>
				  <tr>
					  ...
					  <td align="center"><%=RS("TenHieuQuaCongNghe_ThongTin")%></td>
					  <td align="center"><%=RS("TenSanPham")%></td>
					  <td align="center"><%=RS("TenHoiDong")%></td>
					  <td align="center"><%=RS("GhiChu")%></td>
	  <td><a href="Sua_HieuQuaCongNgheThongTin.asp?MaHieuQuaCongNghe_ThongTin=<%=RS("MaHieuQuaCongNghe_ThongTin")%>">Sửa</a></td>
	  <td><a href="javascript:del(<%=RS("MaHieuQuaCongNghe_ThongTin")%>)">Xóa</a></td>
				  </tr>
			  <%	
			RS.MoveNext
			wend
           ...
           <p align="right"><input type="button" name="nhap" value="Thêm mới" onclick="move()" />
            </p>            
         
<form action="Xoa_HieuQuaCongNgheThongTin.asp" method="get" name="delFrm" id="delFrm">
