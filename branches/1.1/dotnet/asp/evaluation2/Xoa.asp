...
if(loai = "HoiDong") then'Loai o day la loai Hoi Dong
	if(Session("ma_dn")=0 or Session("quyen") <> 1 ) then
		Response.Write("Bạn không có quyền xóa Hội đồng.Xin bạn vui lòng quay lại.")
	else
		dim MaHoiDong
...
'-----------------------------------------Xoa ThanhVien---------------------------------------
	...
	if(Session("ma_dn")=0 or Session("quyen") <> 1 ) then
		Response.Write("Bạn không có quyền xóa thành viên.Xin bạn vui lòng quay lại.")
	else
		dim MaThanhVien
		MaThanhVien = Request.QueryString("MaThanhVien")
		dim kieu
		kieu = Request.QueryString("kieu")
		if(kieu = "" or kieu = null) then
			sqlStr = "EXECUTE XoaThanhVien_PROC @MaThanhVien='"+MaThanhVien+"'"
			conn.execute(sqlStr)
			Response.Redirect("XemDanhSach.asp?loai=ThanhVien")
		else if (kieu = "LoaiTvRaKhoiHD") then
			MaHoiDong = Request.QueryString("MaHoiDong")
			TenHoiDong = Request.QueryString("TenHoiDong")
	
			sqlStr = "EXECUTE LoaiTvRaKhoiHD_PROC @MaThanhVien='"+MaThanhVien+"', @MaHoiDong='"+MaHoiDong+"'"
			conn.execute(sqlStr)
...
'-----------------------------------------Xoa DoanhNghiep---------------------------------------
	...
	if(Session("ma_dn") = 0 and Session("quyen") = 1 ) then
		dim MaDoanhNghiep
		MaDoanhNghiep = Request.QueryString("MaDoanhNghiep")
		sqlStr = "EXECUTE XoaDoanhNghiep_PROC @MaDoanhNghiep='"+MaDoanhNghiep+"'"
		conn.execute(sqlStr)
		Response.Redirect("XemDanhSach.asp?loai=DoanhNghiep")
	else
		Response.Write("Bạn không có quyền xóa Doanh nghiệp.Xin bạn vui lòng quay lại.")
	end if
'-----------------------------------------Xoa SanPham---------------------------------------
else if(loai = "SanPham") then
	if(Session("ma_dn")=0 or Session("quyen") <> 1 ) then
		Response.Write("Bạn không có quyền xóa sản phẩm.Xin bạn vui lòng quay lại.")
	else
		MaSanPham = Request.QueryString("MaSanPham")
		MaDoanhNghiep = Request.QueryString("MaDoanhNghiep")
		TenDoanhNghiep = Request.QueryString("TenDoanhNghiep")
		sqlStr = "EXECUTE XoaSanPham_PROC @MaSanPham='"+MaSanPham+"'"
		conn.execute(sqlStr)
		Response.Redirect("XemDanhSach.asp?loai=SanPham&MaDoanhNghiep="+MaDoanhNghiep+"&TenDoanhNghiep="+TenDoanhNghiep)
	end if
'------------------------------------------------xoa buoc gia cong------------------------
else if(loai = "BuocGiaCong")then
	if(Session("ma_dn")=0 or Session("quyen") <> 1 ) then
		Response.Write("Bạn không có quyền xóa bước gia công.Xin bạn vui lòng quay lại.")
	else
		'Response.Write("Xoa buoc gia cong dang trong qua trinh hoan thien,line 59")
		MaBuocGiaCong = Request.QueryString("MaBuocGiaCong")
		sqlStr = "EXECUTE XoaBuocGiaCong_PROC @MaBuocGiaCong='"+MaBuocGiaCong+"'"
		conn.execute(sqlStr)
		errorStr= "Xóa bước gia công thành công."
		Response.Redirect("XemDanhSach.asp?loai=BuocGiaCong&kieu=all&errorStr="+errorStr)
	end if
'------------------------------------------------xoa thiet bi------------------------
else if(loai = "ThietBi")then
	if(Session("ma_dn")=0 or Session("quyen") <> 1 ) then
		Response.Write("Bạn không có quyền xóa thiết bị.Xin bạn vui lòng quay lại.")
	else
		MaThietBi = Request.QueryString("MaThietBi")
		sqlStr = "EXECUTE XoaMaThietBi_PROC @MaThietBi='"+MaThietBi+"'"
		conn.execute(sqlStr)
		errorStr= "Xóa thiết bị thành công."
		Response.Redirect("XemDanhSach.asp?loai=ThietBi&kieu=all&errorStr="+errorStr)
	end if
end if
end if
end if
