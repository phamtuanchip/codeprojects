...
<title>Cập nhật điểm công nghệ theo công đoạn</title>
<!-- InstanceEndEditable -->
...
On error resume next
	...
	if (CongDoanSize="" or HieuQuaSize="" or MaThanhVien = "") then
	else
		for i=0 to CongDoanSize-1
			MaGiaCong = Request.Form("MaGiaCong"&i)
			for j=0 to HieuQuaSize-1
				MaHieuQua = Request.Form("MaHieuQua"&i&j)
				Pmin = Request.Form("Pmin"&i&j)
				Pmax = Request.Form("Pmax"&i&j)
				min = cInt(Pmin)
				max = cInt(Pmax)
				if(min<1 or min>9 or max<1 or max>9 or min>max) then
					errorStr = "Xin bạn vui lòng nhập lại điểm.Điểm phải lớn hơn 1 và nhỏ hơn 9,và điểm Pmin phải nhỏ hơn điểm Pmax."
					Response.Redirect("PhieuDiemTPCongNghe.asp?errorStr="+errorStr)
				else
					sql = "SELECT COUNT(MaThanhVien) FROM DiemCongNgheTheoCongDoan WHERE MaBuocGiaCong="+MaGiaCong+" AND MaThanhVien="+MaThanhVien+"AND MaHieuQuaCongNghe_KyThuat="+MaHieuQua
					RS2.Open sql,conn
					if(RS2(0)>0) then
						'outStr = "Bạn đã cho điểm rồi,xin vui lòng kiểm tra lại."
						outStr = "Chúc mừng bạn đã sửa điểm thành công."
						StrSql = StrSql + " UPDATE DiemCongNgheTheoCongDoan SET PCDmin="+Pmin+" , PCDmax="+Pmax+" WHERE MaBuocGiaCong="+MaGiaCong+" AND MaThanhVien="+MaThanhVien+"AND MaHieuQuaCongNghe_KyThuat="+MaHieuQua+" "
						'RS3.Open sql2,conn
						'Response.Redirect("PhieuDiemTPCongNghe.asp?errorStr="&outStr&"&MaThanhVien="&MaThanhVien)
					else
					outStr = "Chúc mừng bạn đã cho điểm thành công."
					StrSql = StrSql + " Insert into DiemCongNgheTheoCongDoan (MaBuocGiaCong, MaThanhVien, MaHieuQuaCongNghe_KyThuat, PCDmin, PCDmax) VALUES ("+MaGiaCong+","+MaThanhVien+","+MaHieuQua+","+Pmin+","+Pmax+") "
					'RS.Open sql,conn
					end if
					RS2.Close
				end if
		...
		'Response.Write(CongDoanSize-1)
		'Response.Write(HieuQuaSize-1)
