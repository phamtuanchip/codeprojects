                ...
                <tr>
				  ...
				  <!--#include file="connection.inc" -->
  					<script language="javascript">
						function kiemtra(var inputname)
						{
						 alert(inputname);
						}
					
					</script>
				  <p>Nhập hệ số anpha t cho từng bước gia công của sản phẩm:</p>
				  <% rs.open "select * from sanpham where masanpham ="& Session("ma_sp"),conn%>
				  <p><%=rs("tensanpham")%>&nbsp; </p>
				  <%rs.close()%>
				  <form id="danhsachbgc" name="danhsachbgc" method="post" action="">
				    <table width="100%" border="1">
					 ...
					 <% dem=1
					  ...
					  %>
					  <tr>
                        <td><%=rs("tenbuocgiacong")%>&nbsp;</td>
                        <td><input name="anphat<%=dem%>" type="text" id="anphat<%=dem%>" value="0" size="3" onblur="if ((this.value>1)){alert('Anpha T không được lớn hơn 1'); this.value='0';};kiemtra(var this.name);"/></td>
                      </tr>
					  <%dem = dem +1%>
					  <%rs.movenext()%>
					  <%wend %>
                      ...
                      <tr>
                        ...
                        <td><input name="dem" type="text" id="dem" value="<%=dem-1%>" />
                          <input type="button" name="Submit2" value="tinh"  onclick="tinhtongt ();"/></td>
                      </tr>
                    </table>
                      </form>
				  <p>&nbsp; </p>
