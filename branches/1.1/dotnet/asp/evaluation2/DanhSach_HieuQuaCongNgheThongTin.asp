...
Dim p
	...
	TenTruyCap=session("ten")
		...
		sql="Select DoanhNghiep.MaDoanhNghiep,TenDoanhNghiep from DoanhNghiep inner join users on DoanhNghiep.MaDoanhNghiep=users.MaDoanhNghiep where DoanhNghiep.MaDoanhNghiep="&Session("ma_dn")&" and users.TenTruyCap='" & TenTruyCap & "'"
		Dim RSDoanhNghiep
		Set RSDoanhNghiep= Server.CreateObject("ADODB.Recordset")
		RSDoanhNghiep.open sql ,conn
		
	sqlStr = "SELECT * FROM HieuQuaCongNghe_ThongTin inner join SanPham on SanPham.MaSanPham = HieuQuaCongNghe_ThongTin.MaSanPham inner join HoiDong on HoiDong.MaHoiDong = HieuQuaCongNghe_ThongTin.MaHoiDong where HoiDong.MaDoanhNghiep=" & RSDoanhNghiep(0)
		'Phan trang--------------------------------------------------------------------------------------
		RS.CursorLocation = 3
		'3 nhan vien 1 trang
		RS.PageSize =10
		RS.Open sqlStr, conn
			...
			%>
			<h3 align="center">Danh sách hiệu quả công nghệ thông tin</h3>
			...
			<form action="DanhSach_HieuQuaCongNgheThongTin.asp" method="get" name="viewFrm" id="viewFrm">
