...
<title>Cập nhật điểm thành phần con người theo công đoạn</title>
<!-- InstanceEndEditable -->
...
On error resume next
	...
	for i=0 to CongDoanSize-1
		...
		for j=0 to HieuQuaSize-1
			...
			HCDmin = Request.Form("Pmin"&i&j)
			HCDmax = Request.Form("Pmax"&i&j)
			
				sql2 = "SELECT COUNT(MaThanhVien) FROM DiemConNguoiTheoCongDoan WHERE MaBuocGiaCong="+MaGiaCong+" AND MaThanhVien="+MaThanhVien+"AND MaNhanToConNguoi="+MaHieuQua
				RS2.Open sql2,conn
				if(RS2(0)>0) then
						outStr = "Chúc mừng bạn đã sửa điểm thành công."
						sql = sql + " UPDATE DiemConNguoiTheoCongDoan SET HCDmin="+HCDmin+" , HCDmax="+HCDmax+" WHERE MaBuocGiaCong="+MaGiaCong+" AND MaThanhVien="+MaThanhVien+"AND MaNhanToConNguoi="+MaHieuQua
						'RS3.Open sql2,conn
						'RS3.Close
						'Response.Redirect("PhieuChoDiemTPConNguoiTheoCongDoan.asp?errorStr="&outStr&"&MaThanhVien="&MaThanhVien)
				else
				sql = sql + " Insert into DiemConNguoiTheoCongDoan (MaBuocGiaCong, MaThanhVien, MaNhanToConNguoi, HCDmin, HCDmax) VALUES ("+MaGiaCong+","+MaThanhVien+","+MaHieuQua+","+HCDmin+","+HCDmax+")"
				'RS.Open sql,conn
