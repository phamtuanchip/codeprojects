...
function setValue(page) 
{
	...
	function move(){
		window.location = "Nhap_NhanToConNguoi.asp"
	}
...
function istxtChiTietThongTinEmpty(formtimma) {
      if ( 	formtimma.timma.value == ""){
          alert("Bạn phải nhập mã nhân tố con người cần tìm");
          return false;
       }
	   else {
          return true;
...
dim str
str = "Select TenNhanToConNguoi from NhanToConNguoi"
rstim.Open str,conn
%>
<form name="formtimma" method="post" action="Tim_NhanToConNguoi.asp" onsubmit="return validateForm(this)">
<table>
  <tr align="center">
  	<td>Tìm theo mã nhân tố con người</td>	
...
<form name="formtimten" action="Tim_NhanToConNguoi.asp" method="post">
</form>
<%
dim sqlStr
sqlStr ="select * from NhanToConNguoi inner join SanPham on SanPham.MaSanPham = NhanToConNguoi.MaSanPham inner join HoiDong on HoiDong.MaHoiDong = NhanToConNguoi.MaHoiDong where TenNhanToConNguoi = '"+ Request.Form("timten")+ "' or MaNhanToConNguoi = '"+Request.Form("timma")+"'"
	
	RS.Open sqlStr, conn
		
			%>			<h3 align="center">Kết quả tìm kiếm nhân tố con người</h3><table width="100%" border="1" cellpadding="5" align="center">
			    <tr align="center" >
				 
				  <td align="center"><b>Tên nhân tố con người</b></td>
				  <td align="center"><b>Tên sản phẩm</b></td>
				  <td align="center"><b>Tên hội đồng</b></td>
				  <td align="center"><b>AlphaH</b></td>
			...
			%>
				  <tr>
					  ...
					  <td align="center"><%=RS("TenNhanToConNguoi")%></td>
					  <td align="center"><%=RS("TenSanPham")%></td>
					  <td align="center"><%=RS("TenHoiDong")%></td>
					  <td align="center"><%=RS("AlphaH")%></td>
					 
	  <td><a href="Sua_NhanToConNguoi.asp?MaNhanToConNguoi=<%=RS("MaNhanToConNguoi")%>">Sửa</a></td>
	  <td><a href="javascript:del(<%=RS("MaNhanToConNguoi")%>)">Xóa</a></td>
				  </tr>
			  <%	
			RS.MoveNext
			wend
...
<form action="Xoa_NhanToConNguoi.asp" method="get" name="delFrm" id="delFrm">
