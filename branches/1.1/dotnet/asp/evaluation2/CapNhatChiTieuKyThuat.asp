...
<title>Cập nhật các chỉ tiêu kỹ thuật</title>
<!-- InstanceEndEditable -->
          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  <p>
					...
					Q = cdbl(Request.Form("Q"))
					BetaT = cdbl(Request.Form("BetaT"))
					BetaH = cdbl(Request.Form("BetaH"))
					BetaI = cdbl(Request.Form("BetaI"))
					BetaO = cdbl(Request.Form("BetaO"))
					MaSanPham = cint(Request.Form("MaSanPham"))
					MaHoiDong = cint(Request.Form("MaHoiDong"))
					Nam = Request.Form("Nam")
					
					strCheck = "select MaSanPham, MaHoiDong, Nam from TieuChuanKyThuat where MaSanPham ="&MaSanPham& " and MaHoiDong = "&MaHoiDong&" and Nam="&Nam&""
					RS.open strCheck, conn
					if(RS.eof) then
					strSQL = "insert into TieuChuanKyThuat (MaSanPham, MaHoiDong, Q, BetaT, BetaH, BetaI, BetaO, Nam) values ("&MaSanPham&", "&MaHoiDong&", "&Q&", "&BetaT&", "&BetaI&", "&BetaI&", "&BetaO&", "&Nam&")"
					RS1.open strSQL, conn
					Response.Redirect("ViewAllChiTieuKyThuat.asp")
					else
					Response.Write("Các chỉ số trong năm "&Nam&" đã được nhập, bạn vui lòng chọn lại năm" )
					RS.Close()
					end if
				  %>
