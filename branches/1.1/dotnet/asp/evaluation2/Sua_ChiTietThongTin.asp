...
<title>Sửa chi tiết thông tin</title>
<!-- InstanceEndEditable -->
...
function istxtAlphaIEmpty(formsuachitietthongtin) {
      if ( 	formsuachitietthongtin.alphai.value == ""){
...
function isNumberAlphaI(formsuachitietthongtin) {
    var str = formsuachitietthongtin.alphai.value;
    var re = /^[0-9]/;
    str = str.toString( );
    if (!str.match(re)) {
...
Dim RsSua
Dim str
str = "select * from ChiTietThongTin inner join HoiDong on ChiTietThongTin.MaHoiDong = HoiDong.MaHoiDong inner join SanPham on ChiTietThongTin.MaSanPham = SanPham.MaSanPham where ChiTietThongTin.MaChiTietThongTin = '"+ Request.QueryString("MaChiTietThongTin")+ "'"
set RsSua = Server.CreateObject("ADODB.Recordset")
RsSua.Open str,conn
%>
<center><font face="Arial, Helvetica, sans-serif" color="#006633" size="4">Sửa chi tiết thông tin</font><p>
Chú ý : Bạn không thể sửa những trường khóa có dấu (*)
</center>
<form name="formsuachitietthongtin" id="formsuachitietthongtin" method="post" action="Save_ChiTietThongTin.asp" onsubmit="return validateForm(this)">
  <table width="418" border="0" align="center">
    <tr>
      <td width="172"></td>
      <td width="230"><input name="macttt" type="hidden" id="macttt" value="<%=RsSua("MaChiTietThongTin")%>" readonly>&nbsp;&nbsp;</td>

    </tr>
    <tr>
      <td>T&ecirc;n chi tiết th&ocirc;ng tin </td>
      <td><input name="tencttt" type="text" id="tencttt" value="<%=RsSua("TenChiTietThongTin")%>" size="30"></td>
    </tr>
    <tr>
      <td>Tên sản phẩm</td>
	...
	<tr>
      <td>Alpha I</td>
      <td><input name="alphai" type="text" id="alphai" onblur="if ((this.value>=1)||(this.value<0)){alert('Điểm Alpha I không được nhỏ hơn 0 hoặc lớn hơn 1'); this.value='0';};" value="<%=RsSua("AlphaI")%>" size="30">
      &nbsp;&nbsp;</td>
    </tr>
    <tr>
      <td>Ghi ch&uacute;</td>
