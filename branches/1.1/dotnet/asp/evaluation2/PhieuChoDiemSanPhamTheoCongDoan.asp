	...
	for i=0 to CongDoanSize-1
		...
		for j=0 to HieuQuaSize-1
			...
			HCDmax = Request.Form("Pmax"&i&j)
				...
				sql = "SELECT COUNT(MaThanhVien) FROM DiemConNguoiTheoCongDoan WHERE MaBuocGiaCong="+MaGiaCong+" AND MaThanhVien="+MaThanhVien+"AND MaNhanToConNguoi="+MaHieuQua
				RS2.Open sql,conn
				if(RS2(0)>0) then
					outStr = "Thành viên này đã chấm điểm rồi. Bạn hãy chọn thành viên khác."
				else
				sql = "Insert into DiemConNguoiTheoCongDoan (MaBuocGiaCong, MaThanhVien, MaNhanToConNguoi, HCDmin, HCDmax) VALUES ("+MaGiaCong+","+MaThanhVien+","+MaHieuQua+","+HCDmin+","+HCDmax+")"
				RS.Open sql,conn
