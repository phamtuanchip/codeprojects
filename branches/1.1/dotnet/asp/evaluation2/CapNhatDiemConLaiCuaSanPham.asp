...
<title>Cập nhật điểm còn lại của sản phẩm</title>
<!-- InstanceEndEditable -->
          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  <% 
					...
					<%
					CountTB=Cint(Request.Form("CountTB")) 
					if Cint(Request.Form("CountTB")) =1 	then
					 CountTB=Cint(Request.Form("CountTB")) 
					 else
					 CountTB=Cint(Request.Form("CountTB"))-1
					end if
					i=1
					while i <= Cint(Request.Form("CountTB")) 'CountTB 
						MaThietBi=Request.Form("MaThietBi"&i)
						T=Request.Form("T"&i)
						H=Request.Form("H"&i)						
						N=Request.Form("N"&i)	
						C=Request.Form("C"&i)
						k=Request.Form("k"&i)
						Kt=Request.Form("Kt"&i)
						Kh=Request.Form("Kh"&i)
						Kn=Request.Form("Kn"&i)
						Kc=Request.Form("Kc"&i)
'						Response.Write(MaThietBi)
'						Response.Write("<br>")
						Sql="Update ThietBi Set T=" & T & ",H=" & H & ",N=" & N & ",C=" & C & ",k=" & k & ",Kt=" & Kt & ",Kh=" & Kh & ",Kn=" & Kn & ",Kc=" & Kc & " where MaThietBi=" & MaThietBi & " and MaDoanhNghiep=" & MaDoanhNghiep
						'Response.Write(Sql)
					...
					wend 
				  	...
				  	Response.Write("Bạn đã nhập điểm thành công")
					
					Response.Redirect("PhieuDiemNhapGiaTriConLaiCuaThietBi.asp")
