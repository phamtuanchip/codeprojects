...
<p>&nbsp;</p>     
	...
	rs.CursorLocation = 3;
	 //4 nhan vien 1 trang
	rs.PageSize = 2;
	rs.Open("SELECT NV.ma_nv,NV.ten_nv,CV.ten_chuc_vu FROM NHANVIEN NV inner join CHUCVU CV on NV.ma_chuc_vu = CV.ma_chuc_vu where NV.ma_nv like'" + Request.Form("manv") + "%'" + "and NV.ma_chuc_vu like N'%" + Request.Form("macv") + "%'", oConn);
...
%>
                ...
                <table width="565" border="1" cellpadding="5" cellspacing="0" class="Link">
                  <tr>
                    <td colspan="4" class="tex_de_muc"><div align="center" class="Folder">Kết quả tìm kiếm</div></td>
                  </tr>
                  <tr bgcolor="#CCCCCC">
                    <td width="99" bgcolor="#CCCCCC">Mã nhân viên</td>
                    <td width="163">Họ và tên</td>
                    <td width="135" bgcolor="#CCCCCC">Chức vụ</td>
                    <td bgcolor="#CCCCCC">Kiến tạo</td>
                  </tr>
                  <% 
				  		//rs = oConn.Execute("SELECT NV.ma_nv,NV.ten_nv,CV.ten_chuc_vu FROM NHANVIEN NV inner join CHUCVU CV on NV.ma_chuc_vu = CV.ma_chuc_vu where NV.ma_nv like'" + Request.Form("manv") + "%'" + "and NV.ten_nv like N'%" + Request.Form("tennv") + "%'")
                  ...
                  <TR>
                    <td><%=rs("ma_nv")%> </td>
                    <td><a href="nvDetail.asp?manv=<%=rs("ma_nv")%>"> <%=rs("ten_nv")%> </a></td>
                    <td><%=rs("ten_chuc_vu")%> </td>
                    <td><a href="Editnv.asp?manv=<%=rs("ma_nv")%>">Sửa</a></td>
                  </TR>
                  <%
						rs.MoveNext();
						}
 ...
 // Hien thi phan duyet phan trang
	...
	%>
                <p></p>
                <form name="viewFrm" method="post" action="Searchnv.asp">
                  <p>
