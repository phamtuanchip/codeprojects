    ...
    <tr>
      <td colspan="2" background="../images/bg01.gif"><table width="100%" height="57" border="0" cellpadding="0" cellspacing="0">
        <tr valign="top">
		  ...
		  <% sqltext="select PHIEUNHAN_TB.ma_phieu_nhan,KHACHHANG.ten_kh,NHANVIEN.ten_dang_nhap, datepart(dd,ngay_lap_phieu) ngay," 
					...
					+" order by PHIEUNHAN_TB.ngay_lap_phieu desc"
...
%>        <fieldset>
		  <legend align="center">Danh sách phiếu đã lập; Có (<%=rscount("dem")%>) phiếu đã lập và (<%=rscountitem("dem")%>) thiết bị của khách gửi</legend>
		  ...
		  <table width="100%" border="1" cellpadding="0" cellspacing="0" >
                      <tr> 
                        ...
                        <td width="217">TB Kèm theo</td>
                        <td width="76" nowrap>Ngày lập</td>
                        <td width="64" nowrap>Người lập</td>
                      </tr>
                      <% dem=1
			  ...
			  %>
                        <td><%while (!rs2.EOF){%>
                          |<a  href="../items/item_detail.asp?mathietbi=<%=rs2("ma_tb_kh")%>"><%=rs2("ma_tb_kh")%></a>| 
                          <%rs2.MoveNext;}rs2.Close;%>
                          &nbsp; </td>
                        <td><%=rs("ngay")%>/<%=rs("thang")%>/<%=rs("nam")%></td>
                        <td><div align="center"><%=rs("ten_dang_nhap")%></div></td>
                      </tr>
