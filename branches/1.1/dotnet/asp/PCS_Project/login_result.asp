...
<p>&nbsp;</p>
	...
	var RS;
		...
		Session("username") = Session("username") + Request.Form("username");
		Session("password") = Session("password") + Request.Form("password");
		var sql;
		
		//var Uname = Request.Form("username");
		
		sql = "select * from NHANVIEN where ten_dang_nhap ='" 
		sql = sql + Session("username") + "'"  
		sql = sql + "AND mat_khau ='"  
		sql = sql + Session("password") + "'" ;
		rs = oConn.Execute(sql);
		Session("ten_nv") = Session("ten_nv") + rs("ten_nv");
		Session("ma_nv")=Session("ma_nv")+ rs("ma_nv");
		//Kiem tra da co username va pass nay chua
		if(!rs.EOF){
			Session("login") = true;
			...
			Response.Write("Tên truy nhập hoặc mật khẩu của bạn không đúng.");
			Session("login") = false;
