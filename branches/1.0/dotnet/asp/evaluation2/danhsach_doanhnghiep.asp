          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  <form id="form1" name="form1" method="post" action="" >
                    ...
                    <% if cint(Request.QueryString("ma_dn"))> 0 then ' nếu đã chọn doanh nghiệp sẽ hiện ra%><fieldset >
                    <%
					
					dim rsdoanhnghiep
					set rsdoanhnghiep=server.CreateObject("adodb.recordset")
				 %>
                    <%rsdoanhnghiep.open "select * from doanhnghiep where status=1 and madoanhnghiep="& Request.QueryString("ma_dn"),conn%>
					...
					<%end if ' ket thuc phan ma doanh nghiep >0 %>
                    
				  </form> 
					<table width="93%" border="1" align="center">
                      <tr>
                        <td width="28%"><strong>Tên doanh nghiệp </strong></td>
                        <td width="54%"><strong>Hội đồng chấm điểm </strong></td>
                        <td width="18%"><strong>Thao tác </strong></td>
                      </tr>
					  ...
					  <%while not rs.eof %>
					  <tr>
                        <td><a href="danhsach_doanhnghiep.asp?ma_dn=<%=rs(0)%>" style="text-decoration:none"><% =rs("tendoanhnghiep") %></a>&nbsp;</td>
                        <td>
                          <% 
						rshd.open "select * from hoidong where status=1 and madoanhnghiep='"& rs(0) &"'",conn
						 %>
                          <% while not rshd.eof  %>
                          -
                          <% = rshd("tenhoidong") %>
						 ...
						 wend 
						 rshd.close()%>						  &nbsp;</td>
                        <td><a href="drawappletMain.asp?ma_dn=<%=rs(0)%>">Xem đánh giá</a> </td>
                      </tr>
					  <%
					  rs.movenext()
					  wend
