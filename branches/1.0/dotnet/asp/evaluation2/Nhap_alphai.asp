...
<title>Nhập hệ số Alpha I cho từng bước gia công</title>
<!-- InstanceEndEditable -->
    ...
    if (val) { nm=val.name; if ((val=val.value)!="") {
      if (test.indexOf('isEmail')!=-1) { p=val.indexOf('@');
        ...
        if (test.indexOf('inRange') != -1) { p=test.indexOf(':');
          ...
          if (num<min || max<num) errors+='- '+nm+' phải bằng '+min+'.\n';
    } } } else if (test.charAt(0) == 'R') errors += '- '+nm+' is required.\n'; }
  } if (errors) alert('Bạn gặp phải lỗ sau :\n'+errors);
...
//-->
                  ...
                  <script language="JavaScript" type="text/javascript">
					    ...
					    function kiemtra(diem)
						{
							...
							for (i=1; i<= diem; i ++)
							{
								tong= parseFloat(tong)+parseFloat(document.getElementById("alphai"+i).value)
								//alert(i);
							}
							return tong
						}
				 ...
				 %>
                  ...
                  <p>&nbsp;</p>
                  <p>Nhập hệ số alpha I của sản phẩm : </p>
				  
				 
                  <%
				   if len(Request.Form()) >0 then
				   
					if Request.Form("demso")=1 then 
						tonum=1
					end if
					if Request.Form("demso")>1 then 
						tonum=Request.Form("demso")
					end if
					  
					for i =1 to tonum
					namealpha="alphai"&i
					namebuocgc="machitiettt"&i
					sqlupdate= "update chitietthongtin set alphai="& Request.Form(namealpha)&" where  machitietthongtin="&Request.Form(namebuocgc)&""
					'Response.Write sqlupdate
					'Response.Write " <br>"
					conn.execute sqlupdate
					'Response.Redirect("danhsachphieudiem.asp")
					'set Request.Form("Tổng alpha T")="0"
                  ...
                  <% rs.open "select * from sanpham where masanpham ="&Session("ma_sp")& " and status=1 ",conn%>
                  <p><%=rs("tensanpham")%>&nbsp; </p>
                  <%rs.close()%>
                  <form action="Nhap_alphai.asp" method="post" name="chitietthongtin" id="chitietthongtin" onsubmit="MM_validateForm('Tổng alpha I','','RinRange1:1');return document.MM_returnValue">
                    <table width="100%" border="1">
                      <tr>
                        <td>Tên thông tin </td>
                        <td>Hệ số anpha I </td>
                      </tr>
                      <% rs.open "select * from chitietthongtin where masanpham = "& Session("ma_sp"),conn%>
                      <% dem=1
					  	while not rs.eof 
					  %>
                      <tr>
                        <td><%=rs("tenchitietthongtin")%>&nbsp;
                            <input name="machitiettt<%=dem%>" type="hidden" id="machitiettt<%=dem%>"  value="<%=rs("machitietthongtin")%>"/></td>
                        <td><input name="alphai<%=dem%>" type="text" id="alphai<%=dem%>" value="<%=rs("alphai")%>" size="3" /></td>
                      </tr>
                      <%dem = dem +1%>
                      <%rs.movenext()%>
                      <%wend %>
                      ...
                      <tr>
                        <td><div align="right">
                            <input name="Submit" type="submit"  onmouseover="document.getElementById('Tổng alpha I').value=kiemtra(<%=dem-1%>)" value="  Nhập  "/>
                        </div></td>
                        <td><input name="Tổng alpha I" type="text" id="Tổng alpha I"  size="3" readonly="true" />
                            <input name="demso" type="hidden" id="demso" value="<%=dem-1%>" readonly="1"/></td>
                      </tr>
                    </table>
                  </form>
