...
<title>Xóa hệ số trọng lượng sản phẩm</title>
<!-- InstanceEndEditable -->
          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  <p>
					...
					strCount = "select count(MaHoiDong) as cao from HeSoTrongLuong where MaHeSoTrongLuong_SanPham = "&MaHeSoTrongLuong_SanPham
					dim RSDel
					Set RSDel = Server.CreateObject("ADODB.Recordset")
					strDel = "delete from HeSoTrongLuong where MaHeSoTrongLuong_SanPham = "&MaHeSoTrongLuong_SanPham
					RS.open strCount, conn
					if(not RS.eof) then
					Response.Write("Hiện thời bạn chưa thể xóa hệ số trọng lượng này vì đã có hội đồng sử dụng. Xin liên lạc với người quản trị hệ thống để biết thêm chi tiết")
					'Response.Write(RS("Cao"))
					else
					RSDel.open strDel, conn
					Response.Redirect("DanhSachHeSoTrongLuong.asp")
					end if
				  %>
				    </p>
				  <p>
