...
<%
	On error resume next
		...
		NhanXet = Request.Form("NhanXet")
		
		sql = "SELECT COUNT(ID) FROM ThanhVienNhanXetDoanhNghiep WHERE MaDoanhNghiep="&MaDoanhNghiep&" AND MaSanPham="&MaSanPham&" and MaHoiDong="&MaHoiDong&" AND MaThanhVien="&MaThanhVien
		RS.Open sql,conn
		countValue = RS(0)
		RS.Close
		
		if(countValue>0) then
			sql = "UPDATE ThanhVienNhanXetDoanhNghiep SET NhanXet = N'"&NhanXet&"' WHERE MaDoanhNghiep="&MaDoanhNghiep&" AND MaSanPham="&MaSanPham&" and MaHoiDong="&MaHoiDong&" AND MaThanhVien="&MaThanhVien
		else
			sql = "INSERT INTO ThanhVienNhanXetDoanhNghiep (MaDoanhNghiep, MaSanPham, MaHoiDong, MaThanhVien, NhanXet) VALUES ("&MaDoanhNghiep&", "&MaSanPham&", "&MaHoiDong&", "&MaThanhVien&", N'"&NhanXet&"')"
		end if
		RS.Open sql,conn
		'RS.Close
		
		errorStr = "Bạn đã nhập nhận xét thành công."
		Response.Redirect("NhapNhanXet.asp?MaHoiDong="&MaHoiDong&"&MaThanhVien="&MaThanhVien&"&MaDoanhNghiep="&MaDoanhNghiep&"&MaSanPham=51&errorStr="&errorStr)
%>
