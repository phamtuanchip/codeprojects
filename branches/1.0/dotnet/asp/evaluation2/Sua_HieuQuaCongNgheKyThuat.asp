...
<title>Sửa hiệu quả công nghệ kỹ thuật</title>
<!-- InstanceEndEditable -->
...
Dim RsSua
Dim str
str = "select * from HieuQuaCongNghe_KyThuat inner join HoiDong on HieuQuaCongNghe_KyThuat.MaHoiDong = HoiDong.MaHoiDong inner join SanPham on HieuQuaCongNghe_KyThuat.MaSanPham = SanPham.MaSanPham where HieuQuaCongNghe_KyThuat.MaHieuQuaCongNghe_KyThuat = '"+ Request.QueryString("MaHieuQuaCongNghe_KyThuat")+ "'"
set RsSua = Server.CreateObject("ADODB.Recordset")
RsSua.Open str,conn
%><center><font face="Arial, Helvetica, sans-serif" color="#006633" size="4">Sửa hiệu quả công nghệ kỹ thuật</font>
<p>
Chú ý : Bạn không thể sửa những trường khóa có dấu (*)
</center>
<form name="formsuahieuquacongnghekythuat" id="formsuahieuquacongnghekythuat" method="post" action="Save_HieuQuaCongNgheKyThuat.asp" onSubmit="return validateForm(this)">
  <table width="418" border="0" align="center">
    <tr>
      <td width="172"></td>
      <td width="230"><input name="mahqcnkt" type="hidden" id="mahqcnkt" ac
	  value="<%=RsSua("MaHieuQuaCongNghe_KyThuat")%>" readonly>&nbsp;&nbsp;</td>
    </tr>
    <tr>
      <td>Tên hiệu quả công nghệ kỹ thuật</td>
      <td><input name="tenhqcnkt" type="text" id="tenhqcnkt"
	  value="<%=RsSua("TenHieuQuaCongNghe_KyThuat")%>" size="30" ac></td>
    </tr>
    <tr>
      <td>Tên sản phẩm</td>
      <td><input name="masp" type="text" id="masp"
	  value="<%=RsSua("TenSanPham")%>" size="30" readonly ac>
      &nbsp;&nbsp;*</td>
    </tr>
   <tr>
      <td>Tên hội đồng</td>
      <td><input name="mahd" type="text" id="mahd"
	  value="<%=RsSua("TenHoiDong")%>" size="30" readonly ac>
      &nbsp;&nbsp;*</td>
    </tr>
