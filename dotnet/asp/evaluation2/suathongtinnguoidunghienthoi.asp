        ...
        <tbody>
          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  {
				    if (document.getElementById('Mật khẩu mới').value!=document.getElementById('Mật khẩu lần 2').value) {alert('Mật khẩu ghõ lại phải giống với mật khẩu mới') ;
				  	return false ;
					}
				   return true;
				  }
...
<%
				 if (session("trangthai")=true) then
				   quantricho="Toàn bộ hệ thống"
				   rights=0
				  ' wright=""
				   select case session("quyen")
					   case 1 : if (session("ma_doanhnghiep")=0) then 
					                rights=1
					   			end if
					   ...
					   case  2 : if (session("ma_doanhnghiep")>0) then
					   				rights=3
					   			 end if
					   
					  case  3 : if (session("ma_doanhnghiep")=0) then
					   				rights=5
									quantricho="Toàn bộ hệ thống"
					   			end if
								if (session("ma_doanhnghiep")>0) then
				   ...
				   if  rights>0 then
				   	rs.open "select * from rights where rightId="& rights,conn
					rightshortdec=rs("shortdec")
					rightlongdec=rs("longdec")
					rs.close()
				   end if
				   
				    if (session("ma_doanhnghiep")>0 ) then
						'quantricho="Doanh nghiệp"
						'set rstendoanhnghiep =server.CreateObject("adodb.recordset")
						rs.open "select * from doanhnghiep where madoanhnghiep="& session("ma_doanhnghiep"),conn
					    if not rs.eof then
						 quantricho=rs("tendoanhnghiep")
						end if
						rs.close()
				  ...
				  end if ' neu da dang nhap thanh cong
				  
				  %>
				  <%
				  	if len(request.Form())> 0 then ' bat dau truy van
					 thongbao=""
					 rs.open "select * from users where tentruycap='"&request.Form("tentruycap")&"'",conn
					 'if not rs.eof then
					 if (request.Form("mật khẩu cũ")=rs("matkhau")) then
					 conn.execute "update users set matkhau='"&request.Form("mật khẩu mới")&"' where tentruycap='"&request.Form("tentruycap")&"'"
					 thongbao="Đã thay mật khẩu thành công!"
					 else
					 'messagebox 
					 
					 thongbao="Mật khẩu cũ không đúng ! bạn phải làm lại"
					 end if
					' end if
					 rs.close()
					end if
				  %>
				  <%=thongbao%>
				  <form action="suathongtinnguoidunghienthoi.asp" method="post" name="suathongtin" id="suathongtin" onsubmit="MM_validateForm('Mật khẩu cũ','','R','Mật khẩu mới','','R','Mật khẩu lần 2','','R');return document.MM_returnValue">
				    <table width="100%" border="0">
                      <tr>
                        <td width="240"><div align="right">Tên đăng nhập </div></td>
                        <td width="298"><input name="tentruycap" type="text" id="tentruycap" value="<%=session("ten")%>" size="18" readonly="1"/></td>
                        </tr>
                      <tr>
                        <td><div align="right">Mật khẩu cũ </div></td>
                        <td><input name="Mật khẩu cũ" type="password" id="Mật khẩu cũ" /></td>
                        </tr>
                      <tr>
                        <td><div align="right">Mật khẩu mới </div></td>
                        <td><input name="Mật khẩu mới" type="password" id="Mật khẩu mới" /></td>
                        </tr>
                      <tr>
                        <td><div align="right">Ghõ lại mật khẩu mới </div></td>
                        <td><input name="Mật khẩu lần 2" type="password" id="Mật khẩu lần 2" /></td>
                      </tr>
                      <tr>
                        <td><div align="right"></div></td>
                        <td><input type="submit" name="Submit" value="Ghi lại"  />
                          <input type="submit" name="Submit2" value="Bỏ qua" /></td>
                        </tr>
                    </table>
                      </form>
				  ...
				  <table width="100%" border="0">
                    <tr>
                      <td width="149">Bạn đang thao tác </td>
                      <td width="389"><%=quantricho%>&nbsp;</td>
                      </tr>
                    <tr>
                      <td>Với quyền </td>
                      <td><%=rightshortdec%>&nbsp;</td>
