...
<title>Ghi phiếu điểm sản phẩm</title>
<!-- InstanceEndEditable -->
          ...
          colspan="2" height="100%"><table width="554" border="0" cellspacing="0" cellpadding="0">
                <tr>
				  ...
				  <!--#include file="connection.inc" -->
					  ...
					  <%
						  ...
						  MaThanhVien=Request.Form("MaThanhVien")
				   		'for t = 1 to ( cint(Request.Form("dem"))-1)
						'	tenmi=Request.Form("mi"&t)
						'	MaHeSoTrongLuong_SanPham=Request.Form("mahesotrongluong"&t)
						'	sqlupdate="update hesotrongluong set diem =" & tenmi & " where mahesotrongluong_sanpham="& MaHeSoTrongLuong_SanPham
						'conn.execute sqlupdate
						'	Response.Write(MaHeSoTrongLuong_SanPham & ",  " & tenmi)
						'	Response.Write("<br>")
				  ...
				  <%
						...
						for i = 1 to ( cint(Request.Form("dem"))-1)
						
						tenai=Request.Form("ai"&i)
						MaHeSoTrongLuong_SanPham=Request.Form("mahesotrongluong"&i)
						
						dim rscount 
						set rscount =Server.CreateObject("adodb.recordset")
						rscount.open "select count(*)  from diemsanpham where mahesotrongluong_sanpham=" & MaHeSoTrongLuong_SanPham & " and MaSanPham=" & MaSanPham & " and MaThanhVien="& MaThanhVien ,conn
						
						if (rscount(0)=0) then
							sqlinsert="insert into diemsanpham (masanpham,mathanhvien,diemgoc,mahesotrongluong_sanpham) values ("
							sqlinsert=sqlinsert & MaSanPham & "," & MaThanhVien & "," & tenai & "," & MaHeSoTrongLuong_SanPham &")"
							conn.execute sqlinsert
						else
							 sqlhsupdate="update diemsanpham set diemgoc =" & tenai & " where mahesotrongluong_sanpham="& MaHeSoTrongLuong_SanPham & " and MaSanPham=" & MaSanPham & " and MaThanhVien="& MaThanhVien
							 conn.execute sqlhsupdate
						 end if
						next
				 %>
				 		Nhập điểm thành công !
						<% Response.Redirect("phieudiemsanpham.asp?ma_tv="&MaThanhVien) %>
