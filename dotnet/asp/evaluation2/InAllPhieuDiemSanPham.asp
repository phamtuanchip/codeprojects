...
<% while(not RS.EOF)
	MaThanhVien=Rs(0)
 %>
<h3 align="center">PHIẾU CHO ĐIỂM SẢN PHẨM</h3><form action="Ghi_PhieuDiemSanPham.asp" method="post" name="phieudiem" id="phieudiem" >
  <table width="100%" border="0">
    <tr>
      <td width="20%">Tên sản phẩm:</td>
      <td width="80%"><strong><%=RSSP("TenSanPham")%></strong>
    ...
    <tr>
      ...
      <td><strong><%=RSDN("TenDoanhNghiep")%></strong>
        <input name="madn" type="hidden" id="madn" value="<%=session("ma_dn")%>" />
  </td>
    </tr>
    <tr>
      <td>Họ tên người chấm điểm:</td>
      <td style="border:hidden"><b><%=RS(1)%></b><input name="mahd" type="hidden" id="mahd" value="<%=session("ma_hd")%>" /></td>
    </tr>
  </table>
  ...
  <p align="center">NỘI DUNG CHO ĐIỂM </p>
    <table width="99%" border="1">
      ...
      <%
	    'rs.close()
		RSHSTL.open "select * from hesotrongluong where mahoidong="&session("ma_hd"),conn
		dem=1
	%>
		<% RSHSTL.moveFirst() 
			TongDiem=0
      ...
      <tr>
        <td><div align="center"><%=dem%>
          <input name="mahesotrongluong<%=dem%>" type="hidden" id="mahesotrongluong<%=dem%>" value="<%=RSHSTL("mahesotrongluong_sanpham")%>" />
          </div></td>
        <td title="<%=RSHSTL("GhiChu")%>"><%=RSHSTL("TenHeSoTrongLuong")%></td>
        <td><div align="right">&nbsp;<% =RSHSTL("Diem") %>&nbsp;</div></td>
        <td>
          <%
	    dim rsdiemgoc 
	  	set rsdiemgoc = server.CreateObject("adodb.recordset")
		rsdiemgoc.open "select * from diemsanpham  where mahesotrongluong_sanpham="& RSHSTL("mahesotrongluong_sanpham") & " and MaThanhVien=" & MaThanhVien& " and MaSanPham=" & MaSanPham,conn
	  %>
          <% 
	  ...
	  %>
          <div align="right">&nbsp;<%=diem%>&nbsp;</div>
	    <% rsdiemgoc.close() %>          </td>
		 <% mixai=RSHSTL("Diem")*diem %>
		 
        <td>
          <div align="right">&nbsp;<%=mixai%>&nbsp;
			<% TongDiem=TongDiem+mixai %> 
            </div></td>
      </tr>
      <% dem=dem+1 %>
      <% RSHSTL.movenext()%>
      ...
      <tr>
        ...
        <td align="right">&nbsp;<%=TongDiem %>&nbsp;</td>
      </tr>
      </table>
    <br />
    <table width="303" height="72" border="0" align="right">
      <tr align="right">
        <td align="right">&nbsp;</td>
        <td align="center" valign="bottom"><h5><i><b>Hà Nội ngày.........tháng.........năm...........</b></i> </h5></td>
      </tr>
      <tr>
        <td  align="right">&nbsp;</td>
