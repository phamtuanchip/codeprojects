                ...
                <tr>
				  ...
				  <!--#include file="connection.inc" -->
					   <%if (len(Request.Form())>1) then %>
				        <% 	
					  	if (len(Request.Form("Tên đăng nhập"))>0) then
						dim rscount
						set rscount=server.CreateObject("adodb.recordset")
							rscount.open "select count(*) as kiemtra from [users] where tentruycap='"&Request.Form("Tên đăng nhập")&"'",conn
							 if (rscount("kiemtra")=0 )then
							   sqltext = "insert into [users] (tentruycap,matkhau,quyen,madoanhnghiep) values ("
							   sqltext = sqltext & "N'"&Request.Form("Tên đăng nhập")&"',"
							   sqltext = sqltext & "N'"&Request.Form("Mật khẩu")&"',"
							   sqltext = sqltext & cint(Request.Form("quyen")) &","
							   sqltext = sqltext & (Request.Form("chondn")) &")"
							   'Response.Write sqltext 
							   conn.execute sqltext
							   Response.Redirect("quantri_taikhoan.asp")
							  else
								Response.Write "Tên đăng nhập này đã có yêu cầu nhập tên khác"
								Response.Write "<br/>"
								Response.Write "<a href='quantri_taikhoan.asp'>Quay lại</a>"
							 end if
							rscount.close()
						else %>
