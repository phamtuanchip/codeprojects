...
<td align="left"><span class="style2"><%=RS2(1)%></span></td>
  ...
  <table width="100%" border="1">
    ...
    <tr>
	<% if  RsMaGiaCong.recordCount>0 then
	 		RsMaGiaCong.MoveFirst() 
		end if
	 ...
	 %>
		...
		%>
          ...
          <tr>
            <td align="center" height="20">
              <%=RsMaHieuQua(1)%>
			  <%
			  sql5 = "SELECT HCDmin,HCDmax From DiemConNguoiTheoCongDoan where MaThanhVien=" & RS2(0) & " and MaBuocGiaCong=" & RsMaGiaCong(0) & " and MaNhanToConNguoi=" & RsMaHieuQua(0)
		...
		if (not RsDiem.eof) then 
			diemTaMin = RsDiem("HCDmin")
			diemTaMax = RsDiem("HCDmax")
		else
			diemTaMin = 0
			diemTaMax = 0
		end if
		RsDiem.close
              ...
              <input type="hidden" name="MaHieuQua<%=CongDoanSize%><%=HieuQuaSize%>" value="<%=RsMaHieuQua(0)%>" />              </td>
					  <td height="20"><%=diemTaMin %> &nbsp;</td>
					  <td height="20"><%=diemTaMax %> &nbsp;</td>
		    </tr>
            <%
