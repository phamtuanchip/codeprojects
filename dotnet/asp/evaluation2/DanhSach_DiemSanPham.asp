...
<title>Danh sách điểm sản phẩm</title>
<!-- InstanceEndEditable -->
...
<p>
  ...
  <%
	strSQL = "SELECT dbo.ThanhVien.MaThanhVien, dbo.SanPham.TenSanPham, dbo.HeSoTrongLuong.TenHeSoTrongLuong, dbo.DiemSanPham.DiemGoc FROM dbo.SanPham INNER JOIN dbo.DiemSanPham ON dbo.SanPham.MaSanPham = dbo.DiemSanPham.MaSanPham INNER JOIN dbo.ThanhVien ON dbo.DiemSanPham.MaThanhVien = dbo.ThanhVien.MaThanhVien INNER JOIN dbo.HeSoTrongLuong ON dbo.DiemSanPham.MaHeSoTrongLuong_SanPham = dbo.HeSoTrongLuong.MaHeSoTrongLuong_SanPham INNER JOIN dbo.DoanhNghiep ON dbo.SanPham.MaDoanhNghiep = dbo.DoanhNghiep.MaDoanhNghiep AND dbo.ThanhVien.MaDoanhNghiep = dbo.DoanhNghiep.MaDoanhNghiep WHERE (dbo.DoanhNghiep.MaDoanhNghiep = "&Session("ma_dn")&")"
	RS.open strSQL, conn
%>
</p>
<table width="100%" border="1">
  <tr>
    <td><div align="center">Tên sản phẩm </div></td>
      <td><div align="center">Tên thành viên hội đồng </div></td>
      <td><div align="center">Tên hệ số trọng lượng </div></td>
      <td><div align="center">Điểm gốc </div></td>
    </tr>
	<%if(not RS.EOF) then%>
  <%
  	while not (RS.EOF)
  %>
  <tr>
    <td title="Click vào đây để xem thông tin của sản phẩm <%=RS("TenSanPham")%>"><div align="center"><a href="XemDanhsach.asp?TenSanPham=<%=RS("TenSanPham")%>&loai=SanPham&PhanLoai=ChonLoc"><%=RS("TenSanPham")%></a></div></td>
      <td title="Click vào đây để xem thông tin của <%=RS("TenThanhVien")%>"><div align="center"><a href="XemDanhsach.asp?TenThanhVien=<%=RS("TenThanhVien")%>&loai=ThanhVien&PhanLoai=ChonLoc"><%=RS("TenThanhVien")%></a></div></td>
      <td><div align="center"><%=RS("TenHeSoTrongLuong")%></div></td>
      <td><div align="right"><%=RS("DiemGoc")%></div></td>
    </tr>
  <%
  	RS.MoveNext
	wend
	else
	Response.Write("Hiện tại sản phẩm của doanh nghiệp này chưa có điểm, vui lòng nhập điểm trước khi xem")
	Response.Write("<p>")
	end if
  %>
</table><p>
  <input type="button" name="goback" onclick="history.back()" value="Quay lại" />
</p>
