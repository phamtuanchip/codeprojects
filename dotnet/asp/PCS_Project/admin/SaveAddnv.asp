...
<p>&nbsp;</p>
          ...
          <td width="79%"><table width="98%" height="100%" border="0" cellpadding="00" cellspacing="0">
            <tr>
              ...
              <td width="96%" valign="top"><!-- InstanceBeginEditable name="nhom_thietke" -->
				...
				%>
				    ...
				    <%				
					var str = new String('Hiện đang làm việc');
					strSql = "Select count(*) from NHANVIEN where ma_nv = '" + Request.Form("manv") + "'";
					rs = oConn.Execute(strSql);
					if(rs(0) > 0) {
							Response.Write("Mã nhân viên này đă có rồi");
					} else {
						
					sql= "insert into NHANVIEN values"
					sql = sql + "('" + Request.Form("manv") + "'," 
					sql = sql + "N'" + Request.Form("tennv") + "',"
					sql = sql + "'" + Request.Form("macv") + "',"
					sql = sql + "'" + Request.Form("username") + "',"
					sql = sql + "'" + Request.Form("password") + "',"
					sql = sql + "'" + Request.Form("quyen_dn") + "',"
					sql = sql + "'" + Request.Form("dienthoai") + "',"
			...
			//Response.Write("Select * from NHANVIEN where ma_nv = '" + Request.Form("manv") + "'");
		   %>
</p>
				<p>&nbsp;</p>
				<p>
